...
class ElevationMapBuilder:
    ...
    def update_odometry(self, robot_x: float, robot_y: float):
        ...
        if (src_x_end > src_x_start) and (src_y_end > src_y_start):
            ...
            if (
                overlap.shape
                == new_map[dst_x_start:dst_x_end, dst_y_start:dst_y_end].shape
            ):
    ...
    def add_point_cloud(self, points: np.ndarray):
        ...
        mask = (
            (idx_x >= 0)
            & (idx_x < self.config.grid_size)
            & (idx_y >= 0)
            & (idx_y < self.config.grid_size)
        )
        ...
        np.clip(
            self.map_data,
            self.config.height_clip[0],
            self.config.height_clip[1],
            out=self.map_data,
        )
