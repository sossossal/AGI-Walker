...
class LoadMonitor:
    ...
    def _check_mode_switch(self):
        ...
        if self.current_mode == ControlMode.AI:
            ...
            # 混合模式下，持续正常则切回AI
            if (
                self.consecutive_under_threshold
                >= self.config.recovery_trigger_count * 2
            ):
                self.current_mode = ControlMode.AI
                self.mode_switches += 1
                print("✅ 延迟稳定正常，切回AI控制")
    ...
    def get_control_action(
        self, sensor_data: dict, ai_action: Optional[dict] = None
    ) -> dict:
        """
        获取控制动作
    ...
    def _compute_pid_action(self, sensor_data: dict) -> dict:
        ...
        return {
            "motors": {"hip_left": float(hip_left), "hip_right": float(hip_right)},
            "confidence": 0.8,
        }
...
class SimplePIDController:
    ...
    def compute(self, roll: float, pitch: float, dt: float = 0.033) -> tuple:
        ...
        # 积分限幅
        self.integral_roll = max(
            -self.integral_limit, min(self.integral_limit, self.integral_roll)
        )
        self.integral_pitch = max(
            -self.integral_limit, min(self.integral_limit, self.integral_pitch)
        )
        ...
        # PID输出
        output_roll = (
            self.kp * error_roll + self.ki * self.integral_roll + self.kd * d_roll
        )
        output_pitch = (
            self.kp * error_pitch + self.ki * self.integral_pitch + self.kd * d_pitch
        )
...
if __name__ == "__main__":
    ...
    config = LoadMonitorConfig(
        latency_threshold_ms=20.0, fallback_trigger_count=5, recovery_trigger_count=10
    )
    ...
    sensor_data = {
        "sensors": {
            ...
            "joints": {"hip_left": {"angle": 0.0}, "hip_right": {"angle": 0.0}},
        }
    }
