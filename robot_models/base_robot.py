    ...
    def from_dict(cls, d: dict) -> "RobotConfig":
        ...
        links = [
            LinkConfig(
                ...
                shape=link.get("shape", "box"),
                size=link.get("size", [0.1, 0.1, 0.1]),
                position=link.get("position", [0, 0, 0]),
            )
...
class BipedRobot(BaseRobot):
    ...
    def _default_config(self) -> RobotConfig:
        ...
        return RobotConfig(
            ...
            links=[
                LinkConfig(
                    "torso", "world", mass=5.0, shape="box", size=[0.3, 0.2, 0.4]
                ),
                LinkConfig(
                    "left_thigh", "torso", mass=1.0, shape="capsule", size=[0.08, 0.3]
                ),
                LinkConfig(
                    "right_thigh", "torso", mass=1.0, shape="capsule", size=[0.08, 0.3]
                ),
                LinkConfig(
                    "left_shin",
                    "left_thigh",
...
class QuadrupedRobot(BaseRobot):
    ...
    def _default_config(self) -> RobotConfig:
        ...
        links = [
            LinkConfig("body", "world", mass=5.0, shape="box", size=[0.4, 0.2, 0.3])
        ]
        joints = []
        ...
        for leg in legs:
            links.append(
                LinkConfig(
                    f"{leg}_thigh", "body", mass=0.5, shape="capsule", size=[0.04, 0.15]
                )
            )
            links.append(
                LinkConfig(
            ...
            joints.append(
                JointConfig(f"{leg}_hip", "hinge", [1, 0, 0], (-45, 45), 100, 0.1)
            )
            joints.append(
                JointConfig(f"{leg}_knee", "hinge", [1, 0, 0], (0, 90), 80, 0.1)
            )
...
class WheeledRobot(BaseRobot):
    ...
    def _default_config(self) -> RobotConfig:
        ...
        return RobotConfig(
            ...
            links=[
                LinkConfig(
                    "chassis", "world", mass=4.0, shape="box", size=[0.3, 0.2, 0.1]
                ),
                LinkConfig(
                    "left_wheel",
                    "chassis",
            ...
            joints=[
                JointConfig(
                    "left_wheel", "continuous", [0, 1, 0], (-360, 360), 50, 0.01
                ),
                JointConfig(
                    "right_wheel", "continuous", [0, 1, 0], (-360, 360), 50, 0.01
                ),
            ],
...
def create_robot(robot_type: str, config_path: Optional[str] = None) -> BaseRobot:
    """
    工厂函数：创建机器人实例
...
if __name__ == "__main__":
    ...
    save_robot_config(
        biped.config, str(project_root / "robot_models" / "biped" / "config.json")
    )
    save_robot_config(
        quadruped.config,
        str(project_root / "robot_models" / "quadruped" / "config.json"),
    )
    save_robot_config(
        wheeled.config, str(project_root / "robot_models" / "wheeled" / "config.json")
    )
