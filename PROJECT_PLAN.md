...
- Next-stage readiness reports remain fail-closed by default, but `tools/build_next_stage_readiness_report.py --expected-status blocked` may be used to archive a self-validating blocked evidence snapshot without declaring final readiness.
- CI next-stage readiness artifact collection may try the default ready gate first and then fall back to `--expected-status blocked` only to preserve blocked evidence; validation errors must still fail the job.
- Next-stage readiness Git provenance must use GitHub Actions environment fallbacks when the checkout is detached, so CI artifacts still identify the source branch/ref and commit.
- Next-stage external evidence checklists use `next_stage_external_evidence_checklist.v1` to convert readiness blockers into operator-executable evidence items without changing the readiness pass/fail contract; checklist items preserve route-specific evidence commands, input templates and guide paths for operator handoff, validate item field shapes, validate item artifact paths plus handoff fields as repository-relative paths, include top-level `execution_prerequisites` for Python interpreter and evidence policy, expose `execution_prerequisite_validation_errors` for prerequisite contract drift, expose top-level `validation_errors` for checklist artifact shape/count/status drift, and the CI `next-stage-readiness-artifacts` bundle should retain the checklist beside the authoritative readiness report. Checklist-driven intake status uses `next_stage_external_evidence_status_report.v1` to compare each checklist artifact path against its target status, preserving missing/non-ready external evidence as blocked without replacing the authoritative readiness gate; status reports must reject absolute or parent-directory artifact paths, only read repository-relative artifact paths, and validate each item field shape before a snapshot can be accepted.
- Release-check version metadata uses the root `VERSION` file, and `VERSION` must match `pyproject.toml` `[project].version` so release logs do not drift from package metadata.
...
- External-mainline local file inputs and outputs must be project-root-relative paths; `--project-root` remains the explicit trusted boundary, while absolute paths or parent-directory traversal in managed inputs, local reports, local output artifacts or local override files are rejected before reading, writing or delegating commands.
...
- 2026-07-08: Approved Alpine candidate dependency contract: Web Panel Alpine builds install minimal `libgcc` by default through `WEB_PANEL_APK_PACKAGES`/`AGI_WALKER_WEB_PANEL_APK_PACKAGES` to support the distributed `eclipse-zenoh` install path; remote Docker/Trivy evidence remains required before accepting the candidate.
- 2026-07-08: Approved Alpine candidate native-build contract: Web Panel Alpine builds may install temporary `build-base` through `WEB_PANEL_APK_BUILD_PACKAGES`/`AGI_WALKER_WEB_PANEL_APK_BUILD_PACKAGES` for native Python/Rust package builds, but the virtual `.web-panel-build-deps` package must be removed after pip install before remote Trivy evidence can be accepted.
- 2026-07-08: Approved Alpine candidate build-cache cleanup contract: Web Panel Alpine native builds must remove transient Rust/Cargo and pip build caches from the final image before Trivy evidence is evaluated as production vulnerability posture.
- 2026-07-08: Accepted Alpine candidate vulnerability burn-down evidence: PR #20 proved the candidate in GitHub Actions run `28962518720`, and the merged `main` confirmation run `28967203208` passed `security-preflight` with Python findings `0`, container findings `0`, unresolved findings `0`, accepted findings `0`, active exceptions `0`, review-due exceptions `0`, expired exceptions `0`, and security posture `ready`; obsolete Web Panel no-fix exceptions may be retired from the managed exception input.
- 2026-07-08: Approved external-mainline closeout status refresh: managed `deployment/external_mainline.inputs.json` now skips broad customer collect by default when reusing already-passed customer closure evidence, allowing `run_external_mainline_execution_plan.py --skip-customer-external-bindings-closure` to refresh plan/checklist quickly and promote `industrial_delivery_live_evidence` from missing-input `waiting_external_input` to `ready_to_run` when managed live inputs are complete.
- 2026-07-08: Approved industrial live evidence strict archive gate: `tools/build_industrial_live_evidence_archive_report.py` keeps customer-site smoke optional by default for compatibility, but `--require-customer-site-smoke` now makes a passed customer-site real-device smoke report blocking evidence for industrial signoff archives.
- 2026-07-08: Approved next-stage residual-risk classification hardening: known hardware live diagnostics and browser manual evidence issue ids are classified as external-input actions in `next_stage_readiness_report.v1`, preventing manual/customer-site evidence gaps from being misreported as code-or-config work.
- 2026-07-08: Approved next-stage readiness audit metadata hardening: `next_stage_readiness_report.v1` includes timezone-aware `generated_at` so blocked/ready evidence snapshots can be traced to the actual aggregation run.
...
- 2026-07-08: Approved next-stage external evidence checklist contract: `tools/build_next_stage_external_evidence_checklist.py` may summarize readiness blockers into operator-executable external evidence items while preserving `tools/build_next_stage_readiness_report.py` as the authoritative final gate.
- 2026-07-09: Approved next-stage external evidence checklist CI retention: `next-stage-readiness-artifacts` should include `next_stage_external_evidence_checklist.json` for handoff, while checklist blocked status remains non-gating and the readiness report remains authoritative.
- 2026-07-09: Approved release metadata closeout: root `VERSION` is retained for GitHub release-check logging and is regression-tested against `pyproject.toml` project version.
- 2026-07-09: Approved next-stage external evidence checklist handoff hardening: each unresolved checklist item may include route-specific evidence commands, input templates and guide paths so external operators can execute the next evidence step without changing the readiness report authority or pass/fail semantics.
- 2026-07-09: Approved next-stage external evidence checklist self-validation: generated checklist artifacts must report handoff validation errors for missing commands, malformed handoff arrays or repository-relative template/guide paths that no longer exist, while preserving the authoritative readiness report pass/fail contract.
- 2026-07-09: Approved next-stage external evidence checklist execution prerequisite metadata: generated checklists include top-level `execution_prerequisites` so operators know to use a supported Python interpreter, preferably Python 3.12, and not to replace blocked external evidence with placeholders.
- 2026-07-09: Approved next-stage external evidence checklist prerequisite self-validation: generated checklists expose `execution_prerequisite_validation_errors` and keep the checklist blocked when execution prerequisite metadata is malformed, without changing the authoritative readiness report contract.
- 2026-07-09: Approved next-stage external evidence checklist artifact self-validation: generated checklists expose top-level `validation_errors` for schema, timestamp, collection, summary count and status consistency drift, while preserving `next_stage_readiness_report` as the authoritative release gate.
- 2026-07-09: Approved next-stage external evidence intake status contract: `next_stage_external_evidence_status_report.v1` may validate checklist item artifact paths against their target statuses and archive a blocked or ready intake snapshot, while final release readiness remains governed by `next_stage_readiness_report.v1`.
- 2026-07-09: Approved next-stage status artifact CI hardening: CI may archive a blocked `next_stage_external_evidence_status_report.v1` snapshot with `--expected-status blocked`, but status report self-validation errors must fail the job instead of being ignored.
- 2026-07-09: Approved next-stage status artifact path boundary: `next_stage_external_evidence_status_report.v1` treats checklist `artifact_path` values as repository-relative only and reports validation errors for absolute paths, parent-directory traversal or empty paths.
- 2026-07-09: Approved next-stage status artifact item-shape validation: `next_stage_external_evidence_status_report.v1` self-validation checks item identifiers, paths, booleans, execution scope, issue counts, remaining issue lists and actual status types before accepting blocked or ready snapshots.
- 2026-07-09: Approved next-stage checklist item-shape validation: `next_stage_external_evidence_checklist.v1` self-validation checks item identifiers, artifact paths, statuses, scopes, booleans, issue counts, action/evidence strings and handoff list element types before accepting checklist artifacts.
- 2026-07-09: Approved next-stage checklist artifact path boundary: `next_stage_external_evidence_checklist.v1` treats item `artifact_path` values as repository-relative only and reports validation errors for absolute paths or parent-directory traversal before status-report intake.
- 2026-07-09: Approved Web browser manual evidence file boundary: `web_browser_manual_validation_report` with `schema_version=1.0` treats screenshot/export evidence entries as relative to the manual validation input file, rejects absolute paths or parent-directory traversal, and blocks reports when referenced evidence files are missing.
- 2026-07-10: Approved customer-site smoke evidence file boundary: `customer_site_live_smoke_report` with `schema_version=1.0` treats check `evidence_path` entries as relative to the customer-site smoke input file, rejects absolute paths or parent-directory traversal, and strict evidence-file mode only accepts referenced files that exist under that boundary.
- 2026-07-10: Approved operator delivery evidence path boundary: `operator_delivery_checklist` with `schema_version=1.0` treats template evidence paths and `--set-evidence` overrides as repository-root-relative JSON evidence paths, rejects absolute paths or parent-directory traversal, and preserves required/optional item blocking semantics.
- 2026-07-10: Approved ROS2 typed IDL cutover evidence path boundary: `ros2_typed_idl_cutover_report` with `schema_version=1.0` treats live smoke, typed inventory and rollback evidence as relative paths, rejects absolute paths or parent-directory traversal before reading files, and resolves paths beside the cutover input file before falling back to repository-root template paths.
- 2026-07-10: Approved industrial live evidence archive source path boundary: `industrial_live_evidence_archive_report` with `schema_version=1.0` treats source evidence paths as relative paths, rejects absolute paths or parent-directory traversal before reading files, and resolves paths beside `--inputs-file` before falling back to repository-root template paths.
- 2026-07-10: Approved hardware live closeout source path boundary: `hardware_live_closeout_report` with `schema_version=1.0` treats source evidence paths as relative paths, rejects absolute paths or parent-directory traversal before reading files, and resolves paths beside `--output` before falling back to repository-root template paths.
- 2026-07-10: Approved vendor fault sample closeout path boundary: `vendor_fault_sample_closeout` with `schema_version=1.0` treats source input files and sample `source_evidence` as relative paths, rejects absolute paths or parent-directory traversal before reading files or accepting sample records, and preserves path validation errors in the closeout report.
- 2026-07-10: Approved vendor fault review and promotion path boundary: `vendor_fault_data_review` and `vendor_data_promotion_checklist` with `schema_version=1.0` treat source input files and sample `source_evidence` as relative paths, reject absolute paths or parent-directory traversal before reading files or accepting sample records, and preserve path validation counts/statuses in their reports.
- 2026-07-10: Approved hardware live diagnostics profile path boundary: `hardware_live_diagnostics_checklist` with `schema_version=1.0` treats `--profile-file` as a relative path, rejects absolute paths or parent-directory traversal before reading overrides, and preserves `profile_file_status` plus blockers in the generated checklist.
- 2026-07-10: Approved hardware transport diagnostics source path boundary: `hardware_transport_diagnostics_report` with `schema_version=1.0` treats profile, replay source, fault table and recovery policy paths as relative source paths, rejects absolute paths or parent-directory traversal before reading files, requires referenced sources to exist, and preserves `source_path_statuses` plus blockers in blocked reports.
- 2026-07-10: Approved Web browser validation aggregation source path boundary: `web_browser_validation_closeout` and `web_browser_validation_evidence_pack` with `schema_version=1.0` treat manual, closeout and Playwright report inputs as relative source paths, reject absolute paths or parent-directory traversal before reading files, and preserve `source_path_statuses` plus blockers in blocked reports.
- 2026-07-10: Approved hardwareless acceptance source path boundary: `hardwareless_acceptance_report.v1` treats Godot readiness, hardware live closeout and ROS2 bridge smoke inputs as relative source paths, rejects absolute paths or parent-directory traversal before reading files, and preserves `source_path_statuses` plus blockers in blocked reports.
- 2026-07-10: Approved hardwareless release gate validator path boundary: `hardwareless_release_gate_validation` treats the report input as a repository-relative path, rejects absolute paths or parent-directory traversal before reading files, and preserves `report_path_status` in validation artifacts.
- 2026-07-11: Approved release evidence bundle validator path boundary: `dynamic_godot_release_evidence_bundle_validation.v1` treats the CLI `bundle_index` input as a repository-relative path, rejects absolute paths or parent-directory traversal before reading files, and preserves `bundle_index_path_status` in validation artifacts.
