...
- After a dynamic Godot report wrapper retry, the final retained smoke JSON artifact and report-level `godot_smoke` evidence must agree on `live_verification.flaky_policy.attempts_recorded`, `max_attempts`, and `classification`.
