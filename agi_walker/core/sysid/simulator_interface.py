...
class MockSimulator(SimulatorInterface):
    ...
    def run_trajectory(self, duration=1.0):
        ...
        for i in range(steps):
            ...
            distortion = (self.friction - self.target_friction) * 0.5 + (
                self.damping - self.target_damping
            ) * 0.2
            sim_pos = ideal_pos + distortion

            data["measurements"].append({"state": {"joints": {"hip_left": sim_pos}}})
        return data
...
class RemoteSimulator(SimulatorInterface):
    ...
    def set_params(self, friction, damping):
        payload = {"friction": friction, "damping": damping}
        try:
            # Requires /api/godot/update_physics endpoint (to be implemented)
            res = requests.post(
                f"{self.api_url}/api/godot/update_physics", json=payload
            )
