    ...
    def _get_ai_action(self, sensor_data: dict) -> dict:
        ...
        if self.knowledge_base:
            self.knowledge_base.augment_prompt(
                self.strategy, sensor_data, max_context_length=300
            )
...
def main():
    ...
    parser.add_argument("--small-model", default="phi3:mini", help="小模型名称")
    parser.add_argument("--medium-model", default="mistral:7b", help="中模型名称")
    parser.add_argument("--strategy", default="保持躯干直立，站立稳定", help="控制策略")
    parser.add_argument("--with-vision", action="store_true", help="启用视觉输入")
    ...
    # 运行
    controller.run(duration=args.duration, target_hz=args.hz, verbose=not args.quiet)
