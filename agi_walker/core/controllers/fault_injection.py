...
class FaultInjector:
    ...
    def _inject_sensor_fault(self, data: dict, fault: FaultConfig) -> tuple:
        """注入传感器故障"""
        modified = False
    ...
    def _inject_action_fault(self, data: dict, fault: FaultConfig) -> tuple:
        """注入动作故障"""
        modified = False
    ...
    def _inject_sensor_dropout(self, data: dict, intensity: float, target: str) -> dict:
        """注入传感器丢失"""
        sensors = data.get("sensors", {})
    ...
    def get_delayed_action(self, action: dict, delay_ms: float) -> Optional[dict]:
        """
        处理延迟动作
        ...
        # 返回已到期的动作
        ready_actions = [(t, a) for t, a in self.delay_queue if t <= current_time]

        if ready_actions:
            # 移除已到期的
            self.delay_queue = [(t, a) for t, a in self.delay_queue if t > current_time]
            # 返回最新的到期动作
...
def create_robustness_test_suite() -> List[FaultConfig]:
    ...
    return [
        # 轻度噪声（始终开启）
        FaultConfig(fault_type=FaultType.SENSOR_NOISE, intensity=0.3, probability=1.0),
        # 偶发传感器丢失
        FaultConfig(
            fault_type=FaultType.SENSOR_DROPOUT, intensity=0.5, probability=0.05
        ),
        # 偶发突然扰动
        FaultConfig(
            fault_type=FaultType.SUDDEN_DISTURBANCE, intensity=0.5, probability=0.02
        ),
        # 传感器偏移
        FaultConfig(
