...
class ONNXInferenceEngine:
    ...
    def __init__(self, model_path: str, use_gpu: bool = False):
        ...
        try:
            ...
            logger.info(
                f"ONNX Session loaded from {model_path} with providers {providers}"
            )
    ...
    def benchmark(self, iterations: int = 100):
        ...
        dummy_input = np.random.randn(1, self.session.get_inputs()[0].shape[1]).astype(
            np.float32
        )
        ...
        logger.info(
            f"Benchmark: {iterations} iterations, Avg Latency: {avg_latency:.2f} ms"
        )
        return avg_latency
