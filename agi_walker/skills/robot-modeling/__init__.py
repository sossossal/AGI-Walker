...
class RobotBuilder:
    ...
    def add_torso(
        self, height: float = 0.5, mass: float = 5.0, **kwargs
    ) -> "RobotBuilder":
        """添加躯干
        ...
        part = {
            ...
            "type": "torso",
            "params": {"height": height, "mass": mass, **kwargs},
        }
    ...
    def add_arm_pair(
        ...
        **kwargs,
        ...
        if torso_parts:
            torso_id = torso_parts[0]["id"]
            self.connections.append(
                {"from": torso_id, "to": left_arm["id"], "joint_type": shoulder_joint}
            )
            self.connections.append(
                {"from": torso_id, "to": right_arm["id"], "joint_type": shoulder_joint}
            )
    ...
    def set_joint_limits(
        self, joint_name: str, min_angle: float, max_angle: float
    ) -> "RobotBuilder":
        """设置关节限位"""
        if "joint_limits" not in self.metadata:
            self.metadata["joint_limits"] = {}
        self.metadata["joint_limits"][joint_name] = {"min": min_angle, "max": max_angle}
        return self
