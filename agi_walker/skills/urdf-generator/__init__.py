...
class URDFGenerator:
    ...
    def _connection_to_joint(self, conn: Dict) -> URDFJoint:
        ...
        if joint_type == "revolute":
            ...
            axis = (0, 1, 0)
            limits = {"lower": -1.57, "upper": 1.57, "effort": 100.0, "velocity": 10.0}
        elif joint_type == "prismatic":
            urdf_type = "prismatic"
            axis = (0, 0, 1)
            limits = {"lower": -0.5, "upper": 0.5, "effort": 100.0, "velocity": 1.0}
        else:
            urdf_type = "fixed"
            axis = (0, 0, 0)
            limits = None
    ...
    def export_urdf(self, output_file: str) -> None:
        ...
        try:
            ...
            xml_str = minidom.parseString(xml_str_raw).toprettyxml(
...
def convert_to_urdf(
    input_file: str, output_file: str, generate_meshes: bool = False
) -> None:
    """转换AGI-Walker配置为URDF
...
def convert_to_sdf(input_file: str, output_file: str, world_file: bool = False) -> None:
    """转换为SDF格式 (简化版,基于URDF)
