...
def create_motor_xl430(output_path):
    ...
    # 输出轴（小圆柱）
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.006, depth=0.01, location=(0, 0, 0.028)
    )
    ...
    for angle in [0, 90, 180, 270]:
        ...
        bpy.ops.mesh.primitive_cylinder_add(
            radius=0.0015, depth=0.05, location=(x, y, 0)
        )
    ...
    # 应用材质
    mat = create_material(
        "Motor_Material", (0.2, 0.2, 0.2, 1.0), metallic=0.7, roughness=0.3
    )
    body.data.materials.append(mat)

    # 导出GLTF
    bpy.ops.export_scene.gltf(
        filepath=output_path, export_format="GLTF_SEPARATE", export_selected=True
    )
...
def create_imu_mpu6050(output_path):
    ...
    # 应用材质（绿色PCB）
    mat = create_material(
        "PCB_Material", (0.1, 0.5, 0.2, 1.0), metallic=0.2, roughness=0.7
    )
...
def create_raspberry_pi4(output_path):
    ...
    for i in range(2):
        bpy.ops.mesh.primitive_cube_add(
            size=1, location=(0.02, -0.015 + i * 0.01, 0.003)
        )
        usb = bpy.context.active_object
        usb.scale = (0.006, 0.007, 0.003)

    # 以太网端口
    bpy.ops.mesh.primitive_cube_add(size=1, location=(0.02, 0.01, 0.003))
    eth = bpy.context.active_object
    eth.scale = (0.008, 0.008, 0.003)

    # GPIO针脚（简化为一条）
    bpy.ops.mesh.primitive_cube_add(size=1, location=(-0.015, 0, 0.003))
    ...
    # 应用材质（绿色主板）
    mat = create_material(
        "Board_Material", (0.15, 0.55, 0.25, 1.0), metallic=0.2, roughness=0.6
    )
...
def create_revolute_joint(output_path):
    ...
    # 基座（圆柱）
    bpy.ops.mesh.primitive_cylinder_add(radius=0.012, depth=0.03, location=(0, 0, 0))
    base = bpy.context.active_object
    base.name = "Joint_Base"

    # 轴承（环形）
    bpy.ops.mesh.primitive_torus_add(
        major_radius=0.01, minor_radius=0.003, location=(0, 0, 0.015)
    )
    bpy.context.active_object.name = "Joint_Bearing"

    # 输出臂
    bpy.ops.mesh.primitive_cube_add(size=1, location=(0.015, 0, 0.015))
    ...
    # 应用材质（金属灰）
    mat = create_material(
        "Joint_Material", (0.3, 0.3, 0.3, 1.0), metallic=0.8, roughness=0.4
    )
...
def create_battery_pack(output_path):
    ...
    # 正极端子
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.002, depth=0.003, location=(0.01, 0, 0.008)
    )
    bpy.context.active_object.name = "Battery_Positive_Terminal"

    # 负极端子
    bpy.ops.mesh.primitive_cylinder_add(
        radius=0.002, depth=0.003, location=(-0.01, 0, 0.008)
    )
    bpy.context.active_object.name = "Battery_Negative_Terminal"
    ...
    # 应用材质（蓝色电池）
    mat = create_material(
        "Battery_Material", (0.1, 0.3, 0.7, 1.0), metallic=0.4, roughness=0.5
    )
