...
def create_imu_icon(size=256):
    ...
    for i in range(4):
        ...
        draw.rectangle(
            [x - 2, center_y + height, x + 2, center_y + height + 10],
            fill=(180, 180, 180),
        )
...
def create_controller_icon(size=256):
    ...
    for i in range(2):
        ...
        draw.rectangle(
            [center_x + width // 2, y - 10, center_x + width // 2 + 15, y + 10],
            fill=(180, 180, 180),
            outline=(150, 150, 150),
            width=2,
        )
    ...
    for i in range(gpio_count):
        ...
        draw.rectangle(
            [x - 2, center_y - height // 2 - 8, x + 2, center_y - height // 2],
            fill=(220, 200, 60),
        )
...
def create_joint_icon(size=256):
    ...
    # 基座
    draw.ellipse(
        [center_x - 60, center_y - 20, center_x + 60, center_y + 20],
        fill=(80, 80, 80),
        outline=(60, 60, 60),
        width=3,
    )
    ...
    # 连接臂
    draw.rectangle(
        [center_x, center_y - 15, center_x + 80, center_y + 15],
        fill=(90, 90, 90),
        outline=(70, 70, 70),
        width=2,
    )
    ...
    # 高光
    draw.arc(
        [center_x - 35, center_y - 35, center_x, center_y],
        start=200,
        end=340,
        fill=(150, 150, 150),
        width=3,
...
def create_battery_icon(size=256):
    ...
    # 正极（右侧）
    draw.rectangle(
        [center_x + width // 2, center_y - 8, center_x + width // 2 + 12, center_y + 8],
        fill=(200, 80, 80),
        outline=(160, 60, 60),
        width=2,
    )
