...
class DreamerEnv(gym.Env):
    ...
    def __init__(self, host='127.0.0.1', port=9000, img_size=(64, 64)):
        ...
        self.observation_space = spaces.Dict({
            "image": spaces.Box(low=0, high=255, shape=(img_size[0], img_size[1], 3), dtype=np.uint8),
            "vector": spaces.Box(low=-np.inf, high=np.inf, shape=(24,), dtype=np.float32) # example size
        })
    ...
    def receive_obs(self):
        ...
        if "image" not in obs_json:
            obs_json["image"] = np.zeros((self.img_size[0], self.img_size[1], 3), dtype=np.uint8)
        else:
