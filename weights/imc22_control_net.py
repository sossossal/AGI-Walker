...
class IMC22ControlNet(nn.Module):
    ...
    def __init__(self, state_dim: int = 3, action_dim: int = 3, hidden_dim: int = 16):
        super().__init__()
    ...
    def print_model_info(self):
        ...
        print(
            f"  FP32模型: {mem['total_fp32_bytes']:,} bytes ({mem['total_fp32_bytes'] / 1024:.1f} KB)"
        )
        print(
            f"  INT8模型: {mem['total_int8_bytes']:,} bytes ({mem['total_int8_bytes'] / 1024:.1f} KB)"
        )
