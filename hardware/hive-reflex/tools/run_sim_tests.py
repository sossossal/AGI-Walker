...
class SimulationTester:
    ...
    def run_test(self, binary: str, timeout: int = 10) -> Tuple[bool, str]:
        ...
        try:
            result = subprocess.run(
                [self.qemu, "-nographic", "-machine", "virt", "-kernel", binary],
                capture_output=True,
                timeout=timeout,
                text=True,
            )
    ...
    def run_all_tests(self, test_dir: str):
        ...
        for test_file in test_files:
            ...
            self.results.append(
                {"name": test_file.stem, "passed": passed, "status": status}
            )
...
def main():
    ...
    parser.add_argument(
        "--test-dir", default="build", help="Directory containing test binaries"
    )
    parser.add_argument(
        "--qemu", default="qemu-system-riscv32", help="Path to QEMU executable"
    )
    parser.add_argument(
        "--timeout", type=int, default=10, help="Timeout for each test (seconds)"
    )
    ...
    if args.tests:
        ...
        for test in args.tests:
            ...
            tester.results.append({"name": Path(test).stem, "passed": passed})
    else:
        # 运行所有测试
