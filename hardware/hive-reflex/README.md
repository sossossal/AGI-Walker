...
基于 IMC-22 的模块化脊髓反射控制�?- 分布式感知、本地反射、指令叠加�?
...
**核心特�?**
- 🧠 **边缘智能**: 运行�?IMC-22 (RISC-V + 神经加速器) 上的极简 LSTM 网络
- �?**超低延迟**: 1kHz 控制频率，推理时�?< 50μs
- 🔄 **混合控制**: PID + 神经反射的动态叠加，可调柔顺�?
- 🛡�?**安全机制**: 通信超时检测，自动进入安全模式
- 📡 **分布式架�?*: CAN-FD 总线，支持热插拔和时间同�?
...
hive-reflex/
├── hive_arch.md           # 架构设计文档
├── hive_node_ctrl.c       # 嵌入式控制代�?(C)
├── reflex_net.py          # 神经网络定义和导�?
...
仿真将对比不同柔顺系�?(γ) 下的控制效果，并生成 `simulation_result.png`�?
...
- **γ = 0** (刚性模�?: �?PID 控制，适用于精确轨迹跟�?
- **γ = 0.5** (混合模式): 平衡精度与柔顺�?
- **γ = 1** (柔顺模式): 纯反射控制，适用于动态环境交�?
...
- **通信协议**: CAN-FD (详见架构文档�?4 �?

## ⚠️ 注意事项

1. **C 代码中的 lint 错误**: 由于缺少硬件驱动头文�?(`can_protocol.h`, `imc_driver.h`)，IDE 会报错。这些文件由硬件厂商提供，实际编译时需链接�?
