    ...
    def load_icon(self):
        ...
        try:
            ...
            self.image_obj = self.canvas.create_image(
                self.x + 30, self.y + 30, image=self.photo, tags=("part", self.part_id)
            )
...
class PartsLibraryPanel(ttk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        self.detail_text = tk.Text(
            detail_frame, height=8, wrap=tk.WORD, font=("Arial", 9)
        )
        ...
        ttk.Button(add_frame, text="➕ 添加到画布", command=self.add_to_canvas).pack(
            side=tk.LEFT, padx=5
        )
        ttk.Button(add_frame, text="🔍 查看详情", command=self.view_details).pack(
            side=tk.LEFT, padx=5
        )
    ...
    def load_parts(self):
        ...
        for category, parts in categories.items():
            ...
            for part in parts:
                self.tree.insert(
                    cat_id, "end", text=part["model"], values=(part["id"],)
                )
    ...
    def on_search(self, *args):
        ...
        for category, parts in categories.items():
            ...
            filtered_parts = [
                ...
                for p in parts
                if not search_term
                or search_term in p["model"].lower()
...
- 四足机器人
    ...
    def on_drag_end(self, event):
        ...
        if self.dragging:
            ...
            # 如果鼠标在画布区域内
            if (
                canvas_x <= mouse_x <= canvas_x + canvas_w
                and canvas_y <= mouse_y <= canvas_y + canvas_h
            ):
                # 添加零件到画布
...
class AssemblyCanvas(ttk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        ttk.Button(toolbar, text="🗑️ 删除", command=self.delete_selected).pack(
            side=tk.LEFT, padx=2
        )
        self.connect_btn = ttk.Button(
            toolbar, text="🔗 连接模式", command=self.toggle_connect_mode
        )
        self.connect_btn.pack(side=tk.LEFT, padx=2)
        ttk.Button(toolbar, text="🧹 清空", command=self.clear_canvas).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(toolbar, text="📐 网格", command=self.toggle_grid).pack(
            side=tk.LEFT, padx=2
        )
    ...
    def create_connection(self, from_node, to_node):
        ...
        self.connections.append({"from": from_node, "to": to_node, "line": line})
    ...
    def delete_selected(self):
        ...
        if self.selected_node and self.selected_node in self.nodes:
            ...
            self.connections = [
                conn
                for conn in self.connections
                if conn["from"] != self.selected_node
                and conn["to"] != self.selected_node
            ]
...
class PropertiesPanel(ttk.Frame):
    ...
    def show_properties(self, part_id, part_data):
        ...
        ttk.Label(info_frame, text=f"零件ID: {part_id}").pack(
            anchor=tk.W, padx=5, pady=2
        )
        ttk.Label(info_frame, text=f"型号: {part_data.get('model', 'N/A')}").pack(
            anchor=tk.W, padx=5, pady=2
        )
        ...
        ttk.Checkbutton(
            sync_frame, text="实时同步到Godot", variable=self.auto_sync_var
        ).pack(side=tk.LEFT)

        ttk.Label(sync_frame, text="（需先连接Godot）", foreground="gray").pack(
            side=tk.LEFT, padx=5
        )
        ...
        ttk.Button(button_frame, text="✓ 应用更改", command=self.apply_changes).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(
            button_frame, text="🔄 同步到Godot", command=self.sync_to_godot
...
class FeedbackPanel(ttk.Frame):
    ...
    def __init__(self, parent, app):
        ...
        ttk.Entry(conn_ctrl_frame, textvariable=self.host_var, width=12).pack(
            side=tk.LEFT, padx=2
        )

        ttk.Label(conn_ctrl_frame, text="端口:").pack(side=tk.LEFT, padx=5)
        self.port_var = tk.StringVar(value="9999")
        ttk.Entry(conn_ctrl_frame, textvariable=self.port_var, width=6).pack(
            side=tk.LEFT, padx=2
        )

        self.connect_btn = ttk.Button(
            conn_ctrl_frame, text="🔌 连接", command=self.toggle_connection
        )
        self.connect_btn.pack(side=tk.LEFT, padx=5)

        self.conn_status_label = ttk.Label(
            conn_ctrl_frame, text="● 未连接", foreground="gray"
        )
        ...
        ttk.Button(
            control_frame, text="🔄 同步参数", command=self.sync_parameters
        ).pack(side=tk.LEFT, padx=2)

        # 模式切换
        self.mode_var = tk.StringVar(value="godot")
        ttk.Radiobutton(
            control_frame, text="Godot", variable=self.mode_var, value="godot"
        ).pack(side=tk.LEFT, padx=5)
        ttk.Radiobutton(
            control_frame, text="模拟", variable=self.mode_var, value="mock"
        ).pack(side=tk.LEFT)
        ...
        self.status_label = ttk.Label(
            status_frame, text="状态: 待机", foreground="blue"
        )
    ...
    def get_robot_config(self):
        ...
        for conn in self.app.canvas_panel.connections:
            connections.append({"from": conn["from"], "to": conn["to"]})
