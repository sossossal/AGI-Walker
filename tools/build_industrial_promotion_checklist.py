#!/usr/bin/env python
"""Build a structured industrial-promotion checklist for the current HEAD."""
...
from agi_walker.core.api.release_contracts import (
    ...
    build_extension_execution_actuals,
    build_extension_execution_evidence,
    build_extension_execution_instance,
    build_extension_execution_plan,
    build_extension_execution_schedule,
...
def _default_industrial_build_id(version: str) -> str:
    normalized = re.sub(r"[^A-Za-z0-9._-]+", "-", version)
    return f"build-{normalized}-industrial"
...
def _build_final_command(
    *,
    ...
    approval_notes = approval.get("notes") or "industrial signoff"
    return (
        "python tools/build_release_artifact.py "
        f"--version {version} "
        "--channel industrial "
        f"--build-id {_default_industrial_build_id(version)} "
        "--approval-status approved "
        f"--approved-by {approved_by} "
        f"--approved-at {approved_at} "
        f'--approval-notes "{approval_notes}" '
        "--output test_env/release/release_manifest_industrial.json"
    )
...
def _run_readiness_command(
    *,
    ...
    stdout_lines = [
        f"industrial_release_readiness_written={result.report_path}",
        f"current_version={result.current_version}",
        f"industrial_version={result.industrial_version}",
        f"industrial_release_gate={industrial_preview['release_gate_status']}",
        "industrial_security_preflight="
        f"{industrial_preview['security_release_preflight']['status']}",
        f"industrial_vulnerability_exception_review={review_stdout}",
        "industrial_external_mainline_execution_plan="
        f"{_format_readiness_external_mainline_execution_plan_preview(industrial_preview['external_mainline_execution_plan'])}",
        "industrial_external_mainline_input_checklist="
        f"{_format_readiness_external_mainline_input_checklist_preview(industrial_preview['external_mainline_input_checklist'])}",
        "industrial_worktree_release_blocker="
        f"{_format_readiness_worktree_release_blocker_preview(industrial_preview['worktree_release_blocker'])}",
        ...
        f"industrial_next_actions={len(industrial_preview['next_actions'])}",
    ]
...
def _industrial_preview(readiness_payload: dict[str, Any]) -> dict[str, Any]:
    return next(item for item in readiness_payload["previews"] if item["channel"] == "industrial")
...
def _matches_ready_industrial_manifest(
    ...
    *,
    ...
    if approval_manifest.get("artifact_type") != "release_manifest":
        return False
    if approval_manifest.get("channel") != "industrial":
        return False
    if approval_manifest.get("release_gate_status") != "ready":
        return False
    if approval_manifest.get("version") != industrial_preview.get("version"):
        return False
    manifest_source = approval_manifest.get("release_source", {})
    preview_source = industrial_preview.get("release_source", {})
    return (
...
def _build_security_preflight_step(industrial_preview: dict[str, Any]) -> dict[str, Any]:
    security_preflight = industrial_preview.get("security_release_preflight", {})
...
def _build_worktree_release_blocker_preview(
    ...
    *,
    ...
    preview = (
        dict(industrial_preview.get("worktree_release_blocker", {}))
        if isinstance(industrial_preview.get("worktree_release_blocker"), dict)
        else {}
    )
    ...
    source = (
        dict(industrial_preview.get("release_source", {}))
        if isinstance(industrial_preview.get("release_source"), dict)
        else {}
    )
...
def _build_external_mainline_execution_plan_step(
    ...
    preview = (
        dict(industrial_preview.get("external_mainline_execution_plan", {}))
        if isinstance(industrial_preview.get("external_mainline_execution_plan"), dict)
        else {}
    )
    status = str(preview.get("status") or "missing").strip() or "missing"
    ...
    if not summary:
        summary = (
            "external mainline execution plan status is unavailable."
        )
...
def _build_external_mainline_input_checklist_step(
    ...
    preview = (
        dict(industrial_preview.get("external_mainline_input_checklist", {}))
        if isinstance(industrial_preview.get("external_mainline_input_checklist"), dict)
        else {}
    )
    status = str(preview.get("status") or "missing").strip() or "missing"
    if status == "missing":
...
def _build_customer_delivery_step(industrial_preview: dict[str, Any]) -> dict[str, Any]:
    customer_delivery_surface = industrial_preview.get("customer_delivery_surface", {})
...
def _build_industrial_delivery_step(industrial_preview: dict[str, Any]) -> dict[str, Any]:
    industrial_delivery_gate = industrial_preview.get("industrial_delivery_gate", {})
...
def _build_extension_execution_step(
    industrial_preview: dict[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    extension_support_surface = (
        industrial_preview.get("customer_delivery_surface", {}).get(
            "extension_support_surface",
            {},
        )
        if isinstance(industrial_preview.get("customer_delivery_surface"), dict)
        else {}
    )
    plan = build_extension_execution_plan(extension_support_surface)
    is_done = plan.get("status") == "ready"
    return plan, {
        "id": "extension_execution_plan",
        "category": "extension_execution",
        "title": "附带扩展实施动作计划和值班风险闭环证据",
        "status": "done" if is_done else "pending",
        "required": False,
        "blocking": False,
        ...
        "command": None,
        "ready_to_run": False,
        "depends_on": ["customer_delivery_surface"],
    }


def _build_extension_execution_evidence_step(
    *,
    project_root: str,
    industrial_preview: dict[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    evidence = (
        dict(industrial_preview.get("extension_execution_evidence", {}))
        if isinstance(industrial_preview.get("extension_execution_evidence"), dict)
        else build_extension_execution_evidence(
            project_root=project_root,
            extension_support_surface=(
                industrial_preview.get("customer_delivery_surface", {}).get(
                    "extension_support_surface",
    ...
    is_done = evidence.get("status") == "ready"
    return evidence, {
        "id": "extension_execution_evidence",
        "category": "extension_execution_evidence",
        "title": "附带扩展执行留痕报告（含外部绑定确认）",
        "status": "done" if is_done else "pending",
        "required": False,
        "blocking": False,
        ...
        "command": (
            None
            if is_done
            else "python tools/build_extension_execution_evidence.py --output-root test_env/release_evidence/operations"
        ),
...
def _build_extension_execution_instance_step(
    *,
    project_root: str,
    industrial_preview: dict[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    instance = (
        dict(industrial_preview.get("extension_execution_instance", {}))
        if isinstance(industrial_preview.get("extension_execution_instance"), dict)
        else build_extension_execution_instance(
            project_root=project_root,
            extension_support_surface=(
                industrial_preview.get("customer_delivery_surface", {}).get(
                    "extension_support_surface",
    ...
    is_done = instance.get("status") == "ready"
    return instance, {
        "id": "extension_execution_instance",
        "category": "extension_execution_instance",
        "title": "附带客户实例化扩展执行面",
        "status": "done" if is_done else "pending",
        "required": False,
        "blocking": False,
        ...
        "command": (
            None
            if is_done
            else "python tools/build_extension_execution_instance.py --output test_env/release_evidence/operations/extension_execution_instance.json"
        ),
        "ready_to_run": not is_done,
        "depends_on": ["extension_execution_evidence"],
    }


def _build_extension_execution_schedule_step(
    *,
    project_root: str,
    industrial_preview: dict[str, Any],
) -> tuple[dict[str, Any], dict[str, Any]]:
    schedule = (
        dict(industrial_preview.get("extension_execution_schedule", {}))
        if isinstance(industrial_preview.get("extension_execution_schedule"), dict)
        else build_extension_execution_schedule(
            project_root=project_root,
            extension_support_surface=(
                industrial_preview.get("customer_delivery_surface", {}).get(
                    "extension_support_surface",
    ...
    is_done = schedule.get("status") == "ready"
    return schedule, {
        "id": "extension_execution_schedule",
        "category": "extension_execution_schedule",
        "title": "附带客户窗口排程与 closure 归档面",
        "status": "done" if is_done else "pending",
        "required": False,
        "blocking": False,
        ...
        "command": (
            None
            if is_done
            else "python tools/build_extension_execution_schedule.py --output test_env/release_evidence/operations/extension_execution_schedule.json"
        ),
        "ready_to_run": not is_done,
        "depends_on": ["extension_execution_instance"],
    }
...
def _build_extension_execution_actuals_step(
    *,
    ...
    actuals = (
        dict(industrial_preview.get("extension_execution_actuals", {}))
        if isinstance(industrial_preview.get("extension_execution_actuals"), dict)
        else build_extension_execution_actuals(
            project_root=project_root,
            extension_support_surface=(
                industrial_preview.get("customer_delivery_surface", {}).get(
                    "extension_support_surface",
    ...
    is_done = actuals.get("status") == "ready"
    return actuals, {
        ...
        "ready_to_run": not is_done,
        "depends_on": ["extension_execution_schedule"],
    }
    ...
    *, project_root: str, actuals: dict[str, Any]
    ...
    if status in {"missing", "placeholder", "partial"}:
        ...
        if closure_preview is not None:
            ...
            failed_steps = [
                str(item).strip()
                for item in closure_preview.get("failed_steps", [])
                if isinstance(item, str) and item.strip()
            ]
            if failed_steps:
    ...
    return {
        ...
        "command": command,
        "ready_to_run": bool(command),
        "depends_on": ["extension_execution_instance", "extension_execution_actuals"],
    }
...
def _load_customer_external_bindings_closure_preview(
    ...
    failed_steps = (
        [
            str(item).strip()
            for item in metrics.get("failed_steps", [])
            if isinstance(item, str) and item.strip()
        ]
...
def _build_prerequisite_steps(
    *,
    ...
    source = industrial_preview["release_source"]
    version = industrial_preview["version"]
    steps: list[dict[str, Any]] = []

    approval_done = manifest["release_gate"]["release_approval_ready"] == 1
    steps.append(
        {
            "id": "industrial_approval",
            "category": "approval",
            "title": "补齐 industrial 签核元数据",
            ...
            "summary": (
                ...
                if approval_done
                else "industrial 通道要求 approved_by、approved_at、commit_sha 三项齐全。"
            ),
            "command": None,
            "ready_to_run": False,
            "depends_on": [],
    ...
    steps.append(
        {
            ...
            "category": "git",
            "title": "确认 industrial 签核绑定当前 Git HEAD",
            ...
            "command": None,
            "ready_to_run": False,
            "depends_on": ["industrial_approval"],
        }
    )
    ...
    steps.append(
        {
            "id": "version_tag",
            "category": "git",
            "title": "为当前 HEAD 创建匹配 industrial 版本 tag",
...
def _build_final_step(
    *,
    ...
    if _matches_ready_industrial_manifest(
        approval_manifest,
        industrial_preview=industrial_preview,
    ):
        return {
            "id": "build_industrial_manifest",
            "category": "release",
            "title": "生成 industrial release manifest",
            "status": "done",
            "required": True,
            "blocking": False,
            "summary": (
                "已存在与当前 HEAD 匹配的 ready industrial manifest。"
                + (f" 来源: {approval_manifest_path}" if approval_manifest_path else "")
            ),
            "command": None,
            "ready_to_run": False,
            "depends_on": [],
    ...
    return {
        ...
        "blocking": False,
        "summary": (
            "所有前置项已闭合，可直接执行 industrial builder。"
            if not pending_blockers
            else "前置项未闭合，暂不应执行最终 industrial builder。"
        ),
        "command": command,
...
def _build_summary(
    *,
    ...
    if blocking_steps == 0:
        return (
            "industrial promotion 前置项已闭合，最终 manifest 生成命令可执行。 "
            f"external_mainline={external_mainline_summary}, "
            f"worktree={worktree_summary}"
            f"{checklist_suffix}."
        )
    return (
        f"industrial promotion 仍有 {blocking_steps} 个阻塞前置项。 "
        f"external_mainline={external_mainline_summary}, "
        f"worktree={worktree_summary}"
        f"{checklist_suffix}."
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build an industrial promotion checklist for the current HEAD."
    )
    parser.add_argument("--current-version", default=None)
    parser.add_argument("--industrial-version", default=None)
    ...
    parser.add_argument(
        "--output-root",
        default=str(PROJECT_ROOT / "test_env" / "industrial_promotion"),
    )
    ...
    parser.add_argument(
        "--approval-manifest",
        default=None,
        help="Optional ready industrial release manifest used to import approval metadata.",
    )
    return parser
...
def main(argv: list[str] | None = None) -> int:
    ...
    from agi_walker.ops.promotion import execute_industrial_promotion_checklist
    ...
    print(f"industrial_promotion_checklist_written={result.report_path}")
    print(f"industrial_promotion_gate={result.industrial_preview['release_gate_status']}")
    print(f"industrial_promotion_blocking_steps={payload['blocking_steps']}")
    print(
        "industrial_external_mainline_execution_plan="
    ...
    print(
        "industrial_promotion_ready_to_promote="
