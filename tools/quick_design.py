...
def wizard_step_1_basics():
    ...
    target_height = get_input("目标高度 (m)", 0.4 if robot_type=="quadruped" else 0.6, float)
...
def select_parts(robot_type, scenario, target_height, overrides={}):
    ...
    if overrides.get('torque'):
        ...
        candidates = [m for m in motors if "servo" in m['type'] or "brushless" in m['type']]
        candidates.sort(key=lambda x: x['price'], reverse=True)
        motor = candidates[0]
        motor_torque = parse_value(motor['specs'].get('max_torque', motor['specs'].get('torque', '1.0')))
    ...
    joint_eff = parse_value(joint['specs'].get('efficiency', '0.8').replace('%', '')) / 100.0
    backlash = parse_value(joint['specs'].get('backlash', '0').replace('arcmin', ''))
    stiffness = parse_value(joint['specs'].get('stiffness', '10000'))
    
    print(f"  ⚙️ 关节: {CYAN}{joint['name']}{RESET} (Backlash: {backlash}', Stiffness: {stiffness})")
    ...
    if not s_mat:
        if scenario == "performance":
            ...
            s_mat = next(s for s in structs if "aluminum" in s['type'] or "3d_print" in s['type'])
    ...
    specs = {
        ...
        'joint_damping': overrides.get('damping') or (1.0 if scenario == "performance" else 0.5),
        'max_velocity': overrides.get('speed') or (10.0 if scenario == "performance" else 5.0),
        ...
        'stiffness': stiffness,
        'backlash': backlash,
        'thermal_resistance': parse_value(motor['specs'].get('thermal_resistance', '1.0')),
        'continuous_torque': parse_value(motor['specs'].get('continuous_torque', str(motor_torque*0.6)).replace('Nm', '')),
...
def wizard_step_4_build(robot_type, name, specs):
    ...
    # 躯干 (包含电池和控制器)
    torso_mass = specs['total_mass'] - (specs['joint_mass'] * (6 if robot_type=="biped" else 12)) 
...
def wizard_step_5_export(robot, bom_list, cost, specs):
    ...
    extra_metadata = {
        ...
        "advanced_dynamics": {
            "stiffness": robot.physics_overrides.get('stiffness', 10000) if hasattr(robot, 'physics_overrides') else specs.get('stiffness'),
...
def main():
    ...
    parser.add_argument('--scenario', choices=['eco', 'performance', 'custom'], help='Usage scenario')
    ...
    parser.add_argument('--material', choices=['aluminum', 'carbon_fiber', 'plastic'], help='Structure material [Override]')
    ...
    if args.non_interactive:
        ...
        if not all([args.type, args.name, args.scenario, args.height]):
             ...
             # 这里强校验
             sys.stderr.write("Error: Missing required args for non-interactive mode: --type, --name, --scenario, --height\n")
             sys.exit(1)
    ...
    print("="*60)
    if args.non_interactive:
        print(f"OUTPUT_JSON_PATH:{project_root / 'configs' / 'generated' / f'{r_name}.json'}")
