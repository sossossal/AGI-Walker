...
class CodeGeneratorRole(BaseRole):
    ...
    def execute(self, command: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if any(k in cmd for k in ["移动", "运动", "controller"]):
            ...
            return self._gen(
                "3D 玩家移动脚本" if is_3d else "2D 玩家移动脚本",
            ...
            return self._gen(
                "血量系统", self.lib.get("health_system"), "health_system.gd"
            )
            ...
            return self._gen(
                "有限状态机", self.lib.get("state_machine"), "state_machine.gd"
            )
            ...
            return self._gen(
                "战斗系统", self.lib.get("combat_system"), "combat_system.gd"
            )

        elif any(k in cmd for k in ["技能", "冷却", "skill", "cooldown"]):
            return self._gen(
                "技能冷却系统", self.lib.get("skill_system"), "skill_system.gd"
            )
            ...
            return self._gen(
                "摄像机抖动", self.lib.get("camera_shake"), "camera_shake.gd"
            )

        elif any(k in cmd for k in ["单例", "autoload", "全局管理"]):
            return self._gen(
                "游戏管理单例", self.lib.get("game_manager"), "game_manager.gd"
            )
