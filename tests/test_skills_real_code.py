...
class TestSkillMetadata:
    ...
    def test_skill_metadata_structure(self) -> None:
        ...
        metadata = {
            "skills": [
                {"name": "robot-modeling", "type": "modeling", "enabled": True},
                {
                    "name": "parameter-optimizer",
                    "type": "optimization",
                    "enabled": True,
                },
                {"name": "urdf-generator", "type": "generation", "enabled": True},
            ]
        }
...
class TestSkillDataHandling:
    ...
    def test_skill_output_generation(self) -> None:
        """测试：技能输出生成"""
        outputs = {
            "robot-modeling": {"model": "model_data", "status": "success"},
            "parameter-optimizer": {"optimized_params": {"param1": 1.0}, "score": 0.95},
            "urdf-generator": {"urdf_content": "<urdf>...</urdf>", "status": "success"},
        }
...
class TestSkillErrorHandling:
    ...
    def test_skill_configuration_validation(self) -> None:
        """测试：技能配置验证"""
        valid_config = {"model_type": "quadruped", "num_legs": 4}
    ...
    def test_sequential_skill_execution(self) -> None:
        ...
        for step_name, step_data in steps:
            execution_log.append(
                {"step": step_name, "data": step_data, "status": "success"}
            )
