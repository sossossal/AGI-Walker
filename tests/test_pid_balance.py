...
class PIDBalanceTest:
        ...
        while time.time() - start_time < duration:
            ...
            if sensor:
                ...
                if int(elapsed) % 5 == 0 and len(tilts) % 30 == 0:
                    print(
                        f"[{elapsed:5.1f}s] Roll: {roll:5.1f}° Pitch: {pitch:5.1f}° 倾斜: {tilt:5.1f}°"
                    )
    ...
    def _calculate_stats(
        self, test_name, tilts, rolls, pitches, heights, fell, fall_time, duration
    ) -> Dict:
        """计算统计数据"""

        if not tilts:
            return {"test": test_name, "fell": True, "duration": 0, "score": 0}
    ...
    def compare_tests(self):
        ...
        sorted_results = sorted(
            self.test_results, key=lambda x: x["score"], reverse=True
        )
        ...
        for i, result in enumerate(sorted_results, 1):
            print(
                f"{i:<5} {result['test']:<30} {result['score']:>6.1f}/100 "
