...
def test_dynamic_godot_release_evidence_bundle_validator_rejects_external_bundle_path(
    tmp_path: Path,
) -> None:
    closeout_path, gate_path, readiness_path = _write_static_release_bundle_inputs(
        tmp_path
    ...
    for entry in index["artifacts"]:
        if entry["key"] == "delivery_gate":
            entry["bundle_path"] = entry["source_path"]
            break
    index_path.write_text(json.dumps(index), encoding="utf-8")
    ...
    assert payload["status"] == "invalid"
    assert (
        "artifacts.delivery_gate bundle_path must be relative and stay within "
        "bundle root"
    ) in payload["errors"]
...
def test_report_smoke_wrapper_retries_auto_port_startup_timeout(
    ...
    retained_smoke = json.loads(smoke_output.read_text(encoding="utf-8"))
    assert (
    ...
    assert (
        result["report_summary"]["live_verification"]["flaky_policy"][
            "classification"
        ]
        == "passed_after_retry"
    )
...
def test_report_smoke_wrapper_syncs_failed_retry_to_retained_smoke(
    tmp_path: Path,
    monkeypatch,
) -> None:
    spec = importlib.util.spec_from_file_location("dynamic_godot_report", REPORT_TOOL)
    ...
    assert result["returncode"] == 1
    assert result["retried"] is True
    assert [attempt["port"] for attempt in result["attempts"]] == [25567, 25568]
    assert (
        result["live_verification"]["flaky_policy"]["classification"]
        == "failed_after_retry"
    )
