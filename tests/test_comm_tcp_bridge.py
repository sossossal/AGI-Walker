...
class TestTcpZenohBridgeInit:
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
...
class TestTcpServerManagement:
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    @patch("python_api.comm.tcp_zenoh_bridge.socket.socket")
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_start_sets_running_flag(self, mock_zenoh_class) -> None:
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_stop_clears_running_flag(self, mock_zenoh_class) -> None:
...
class TestConnectionHandling:
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_connection_timeout_handling(self, mock_zenoh_class) -> None:
        """测试：连接超时处理"""
        # Arrange
        mock_zenoh_instance = MagicMock()
        mock_zenoh_class.return_value = mock_zenoh_instance
...
class TestErrorHandling:
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_socket_bind_error(self, mock_zenoh_class) -> None:
        """测试：Socket 绑定错误"""
        # Arrange
        mock_zenoh_instance = MagicMock()
        mock_zenoh_class.return_value = mock_zenoh_instance
...
class TestThreadManagement:
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_threads_created_on_start(self, mock_zenoh_class) -> None:
        """测试：启动时创建线程"""
        # Arrange
        mock_zenoh_instance = MagicMock()
        mock_zenoh_class.return_value = mock_zenoh_instance
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_threads_stopped_gracefully(self, mock_zenoh_class) -> None:
        """测试：线程优雅停止"""
        # Arrange
        mock_zenoh_instance = MagicMock()
        mock_zenoh_class.return_value = mock_zenoh_instance
...
class TestBridgeIntegration:
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_full_lifecycle(self, mock_zenoh_class) -> None:
        """测试：完整生命周期"""
        # Arrange
        mock_zenoh_instance = MagicMock()
        mock_zenoh_class.return_value = mock_zenoh_instance
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_restart_bridge(self, mock_zenoh_class) -> None:
        """测试：重启桥接器"""
        # Arrange
        mock_zenoh_instance = MagicMock()
        mock_zenoh_class.return_value = mock_zenoh_instance
    ...
    @patch("python_api.comm.tcp_zenoh_bridge.ZenohInterface")
    def test_configuration_persistence(self, mock_zenoh_class) -> None:
        """测试：配置持久性"""
        # Arrange
        mock_zenoh_instance = MagicMock()
        mock_zenoh_class.return_value = mock_zenoh_instance
