...
class TestSkillsBrowserInitialization:
    ...
    def test_skills_panel_init_structure(self) -> None:
        ...
        # 模拟 skills_loader
        mock_loader = Mock()
        mock_loader.get_categories.return_value = ["optimization", "modeling", "generation"]
...
class TestSkillsSearchFunctionality:
    ...
    def test_search_empty_returns_all_skills(self) -> None:
        ...
        # 模拟搜索逻辑
        search_text = ""
        results = skills if not search_text else [
            s for s in skills 
    ...
    def test_search_by_description(self) -> None:
        ...
        results = [
            s for s in skills 
            if search_text in s["description"]
        ]
...
class TestSkillsCategoryFilter:
    ...
    def test_filter_by_category_all(self) -> None:
        ...
        category = "所有分类"
        filtered = skills if category == "所有分类" else [
    ...
    def test_combined_search_and_filter(self) -> None:
        """测试: 同时搜索和过滤"""
        skills = [
            {"name": "robot-modeling", "category": "modeling", "description": "机器人模型"},
            {"name": "robot-optimizer", "category": "optimization", "description": "机器人优化"},
            {"name": "urdf-generator", "category": "generation", "description": "URDF生成"},
        ]
...
class TestSkillsListOperations:
    ...
    def test_skills_list_item_count(self) -> None:
        """测试: Skills 列表项计数"""
        skills = [
            {"name": f"skill_{i}", "category": f"cat_{i%3}"}
            for i in range(10)
        ]
...
class TestGUIActions:
    ...
    def test_refresh_skills_list(self) -> None:
        ...
        assert refresh_result["refreshed"] == True
...
class TestUIStateManagement:
    ...
    def test_button_enabled_state_on_skill_selection(self) -> None:
        ...
        assert ui_state["open_dir_btn"]["enabled"] == True
        assert ui_state["view_api_btn"]["enabled"] == True
    ...
    def test_button_disabled_state_on_deselection(self) -> None:
        ...
        assert ui_state["open_dir_btn"]["enabled"] == False
