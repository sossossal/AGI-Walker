...
TARGET_HEIGHT = 0.45
EXPECTED_LINKS = ["torso_1", "thigh_left_2", "shin_left_3", "thigh_right_4", "shin_right_5"]
...
def step_2_validate_urdf_structure(urdf_path):
    ...
    try:
        ...
        missing_links = [l for l in EXPECTED_LINKS if not any(l in found for found in links)]
        if missing_links:
        ...
        missing_joints = [j for j in EXPECTED_JOINTS if not any(j in found for found in joints)]
        if missing_joints:
