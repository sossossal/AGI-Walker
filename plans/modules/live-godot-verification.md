...
- 2026-05-20: Re-ran the remaining golden live fixtures `robot_dynamic_fixed_pair.json` and `robot_dynamic_quadruped.json` with `D:\迅雷下载\Godot\Godot.exe`. The fixed pair, quadruped and previously verified biped delivery gates all passed at `godot_verified`, each reported `static_node_tree_manifest_path_map_mismatch_count=0` and `mechanical_behavior_complete_count=1`, and `test_env/dynamic_godot_live/local/dynamic_godot_golden_live_readiness.json` reported `status=ready`, `proven_level=godot_verified` and no residual risks. The gate validator passed across all three gates with `--require-passed --require-complete --expect-level godot_verified --expect-success-count 3 --expect-passed-true-count 3 --expect-summary-count delivery_godot_verified_count=3`.
- 2026-05-31: Added workflow-contract regression coverage for opt-in live/external CI jobs. The dynamic Godot live job remains manual/scheduled, but tests now require its trigger condition, discovery path, `GODOT_EXECUTABLE` guard, artifact upload path and retention policy to remain explicit.
