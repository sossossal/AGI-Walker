...
- 2026-07-10: Hardened customer-site live smoke evidence validation. Check `evidence_path` entries must remain relative to the smoke input file directory, cannot traverse outside it, and strict evidence-file mode resolves files only within that boundary.
- 2026-07-10: Hardened operator delivery checklist evidence validation. Template evidence paths and `--set-evidence` overrides must remain repository-root-relative JSON evidence paths, cannot traverse outside the repository, and keep required/optional blocking semantics intact.
- 2026-07-10: Hardened ROS2 typed IDL cutover evidence validation. Cutover `evidence` paths must be relative, cannot use absolute paths or parent-directory traversal, and are resolved beside the cutover input file before falling back to repository-root template paths.
- 2026-07-10: Hardened industrial live evidence archive source validation. Archive source evidence paths must be relative, cannot use absolute paths or parent-directory traversal, and resolve beside `--inputs-file` before falling back to repository-root template paths.
- 2026-07-10: Hardened hardware live closeout source validation. Closeout source evidence paths must be relative, cannot use absolute paths or parent-directory traversal, and resolve beside `--output` before falling back to repository-root template paths.
- 2026-07-10: Hardened vendor fault sample closeout path validation. Source input files and sample `source_evidence` paths must be relative, cannot use absolute paths or parent-directory traversal, and preserve path validation errors in the closeout report.
- 2026-07-10: Hardened vendor fault review and promotion path validation. Review/promotion source input files and sample `source_evidence` paths must be relative, cannot use absolute paths or parent-directory traversal, resolve beside `--output` before falling back to repository-root templates, and preserve path validation counts/statuses in the generated reports.
- 2026-07-10: Hardened hardware live diagnostics checklist profile validation. Optional `--profile-file` must be relative, cannot use absolute paths or parent-directory traversal, resolves beside `--output` before falling back to repository-root templates, and preserves `profile_file_status` plus blockers in the generated checklist.
- 2026-07-10: Hardened hardware transport diagnostics source validation. Profile, replay source, fault table and recovery policy paths must be relative, cannot use absolute paths or parent-directory traversal, must exist before diagnostics reads them, and preserve `source_path_statuses` plus blockers in blocked reports.
- 2026-07-10: Hardened Web browser validation aggregation source validation. Closeout and evidence-pack source report paths must be relative, cannot use absolute paths or parent-directory traversal, resolve beside `--output` before falling back to repository-root paths, and preserve `source_path_statuses` plus blockers in blocked reports.
- 2026-07-10: Hardened hardwareless acceptance source validation. Godot readiness, hardware live closeout and ROS2 bridge smoke input paths must be relative, cannot use absolute paths or parent-directory traversal, resolve beside `--output` before falling back to repository-root paths, and preserve `source_path_statuses` plus blockers in blocked reports.
- 2026-07-10: Hardened hardwareless release gate validator report path validation. The validator report input must be repository-relative, cannot use absolute paths or parent-directory traversal, and preserves `report_path_status` in the structured validation artifact.
