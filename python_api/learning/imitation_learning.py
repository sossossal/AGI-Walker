...
class ImitationLearner:
    ...
    def collect_expert_demos(
        self, expert_policy, n_episodes: int = 100, save_path: Optional[str] = None
    ):
        """
        从专家策略收集演示
        ...
        trajectories = rollout.rollout(
            expert_policy, venv, rollout.make_sample_until(min_episodes=n_episodes)
        )
...
class BehaviorCloning(ImitationLearner):
    ...
    def train(
        ...
        # 训练
        self.bc_trainer.train(n_epochs=n_epochs, log_interval=1, progress_bar=True)
    ...
    def evaluate(self, n_episodes: int = 10):
        ...
        mean_reward, std_reward = evaluate_policy(
            self.bc_trainer.policy, venv, n_eval_episodes=n_episodes, deterministic=True
        )
