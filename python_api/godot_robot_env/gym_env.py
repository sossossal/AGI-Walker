...
class GodotRobotEnv(gym.Env):
    ...
    def _randomize_dynamics(self):
        ...
        self.current_dynamics["motor_strength"] = random.uniform(
    ...
    def _parse_sensor_data(self, data: Dict) -> Dict:
        ...
        return {
            ...
            "imu_angular_vel": np.array(
                imu.get("angular_vel", [0, 0, 0]), dtype=np.float32
            ),
            "imu_linear_acc": np.array(
                imu.get("linear_acc", [0, 0, 9.81]), dtype=np.float32
            ),
            "joint_angles": np.array(
                joints.get("angles", [0, 0, 0, 0]), dtype=np.float32
            ),
            "joint_velocities": np.array(
                joints.get("velocities", [0, 0, 0, 0]), dtype=np.float32
            ),
            "joint_torques": np.array(
                joints.get("torques", [0, 0, 0, 0]), dtype=np.float32
            ),
            "foot_contacts": np.array(
                [contacts.get("foot_left", False), contacts.get("foot_right", False)],
                dtype=np.int8,
            ),
    ...
    def set_physics_params(self, params: Dict):
        """动态修改物理参数"""
        command = {"type": "set_physics", "params": params}
        self._send_command(command)

    def load_robot_from_parts(self, parts_list: list):
        """从零件列表构建机器人"""
        command = {"type": "build_robot", "parts": parts_list}
        self._send_command(command)
