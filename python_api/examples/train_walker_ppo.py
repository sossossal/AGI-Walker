...
def create_env():
    ...
    # 物理环境配置
    physics_config = {"gravity": 9.81, "ground_friction": 0.8, "air_density": 1.225}
...
def train_walker(
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=10000, save_path=save_path, name_prefix="walker_ppo"
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--timesteps", type=int, default=100000, help="训练步数（默认：100000）"
    )
    parser.add_argument(
        "--model-path",
        type=str,
        default="./models/walker_ppo/walker_ppo_final",
        help="模型路径（测试模式时使用）",
    )
    parser.add_argument(
        "--episodes", type=int, default=10, help="测试回合数（默认：10）"
    )
