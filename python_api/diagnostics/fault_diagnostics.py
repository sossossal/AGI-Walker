    ...
    def estimate_remaining_life(self) -> float:
        ...
        if len(self.stress_history) > 10:
            ...
            if recent_stress > 1.0:
                current_wear_rate *= (
                    1 + (recent_stress - 1) * self.wear_profile.stress_multiplier
                )
            ...
            return (
                remaining_wear / current_wear_rate
                if current_wear_rate > 0
                else float("inf")
            )
...
class FaultDiagnostics:
    ...
    def _check_high_wear_rate(self, component: Component) -> Optional[Dict]:
        """检查高磨损率"""
        if (
            component.operating_hours > 10
            and component.wear_level / component.operating_hours > 0.0005
        ):
            return {
                "severity": "WARNING",
    ...
    def _check_maintenance_due(self, component: Component) -> Optional[Dict]:
        ...
        hours_since_maintenance = (
            component.operating_hours - component.last_maintenance_hours
        )
    ...
    def get_diagnostics_report(self) -> str:
        ...
        report.append("-" * 70)
        report.append(
            f"{'组件':<20} {'运行时间':<12} {'磨损':<10} {'健康':<10} {'剩余寿命':<15}"
        )
        report.append("-" * 70)
        ...
        if urgent_items:
            ...
            for item in urgent_items:
                report.append(
                    f"  [{item['urgency']}] {item['component']}: {item['action']}"
                )
