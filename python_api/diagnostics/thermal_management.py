...
class ThermalComponent:
    ...
    def update_temperature(self, power_w: float, ambient_temp: float, dt: float):
        ...
        self.temperature = self.temperature + alpha * (
            steady_state_temp - self.temperature
        )
...
class ThermalManager:
    ...
    def _update_throttling(self, critical_components: List):
        ...
        # 有过热组件
        critical_count = sum(
            1 for _, status in critical_components if status["level"] == "CRITICAL"
        )
    ...
    def check_cooling_requirements(self) -> Dict:
        ...
        for component in self.components:
            avg_temp = (
                np.mean(self.temp_history[component.name][-100:])
                if len(self.temp_history[component.name]) > 0
                else 25.0
            )
    ...
    def get_thermal_report(self) -> str:
        ...
        report.append("\n组件温度:")
        report.append(
            f"{'组件':<15} {'当前温度':<12} {'状态':<10} {'余量':<10} {'利用率':<10}"
        )
        report.append("-" * 70)
