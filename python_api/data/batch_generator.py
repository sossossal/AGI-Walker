...
class BatchDataGenerator:
    ...
    def run_single_episode(self, episode_config: EpisodeConfig) -> Dict:
        ...
        try:
            ...
            # 创建模拟器
            sim = EnvironmentBalanceSimulator(episode_config.robot_params, env_config)
            ...
            return {"episode_id": episode_id, "success": False, "error": str(e)}
        ...
        with ProcessPoolExecutor(max_workers=self.config.num_workers) as executor:
            ...
            with tqdm(total=len(remaining_configs), desc="生成进度") as pbar:
                for future in as_completed(futures):
                    ...
                    # 定期保存断点
                    if (
                        len(self.completed_episodes) % self.config.checkpoint_interval
                        == 0
                    ):
    ...
    def generate_report(self) -> Dict:
        ...
        report = {
            ...
            "success_rate": len(self.completed_episodes)
            / self.config.num_episodes
            * 100,
            "total_time_seconds": self.total_time,
            "total_steps": self.total_steps,
            "avg_time_per_episode": self.total_time
            / max(len(self.completed_episodes), 1),
            "episodes_per_hour": (
                len(self.completed_episodes) / (self.total_time / 3600)
        ...
        print(
            f"总时间: {report['total_time_seconds']:.1f}s ({report['total_time_seconds']/60:.1f}分钟)"
        )
