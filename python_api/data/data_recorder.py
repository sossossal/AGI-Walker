...
class DataRecorder:
    ...
    def record_state(self, timestamp: float, state: Dict):
        ...
        参数:
            ...
            state: 状态字典
        """
        self.data["timeline"].append({"timestamp": timestamp, "state": state})

        self.record_count += 1
        self.last_timestamp = timestamp

    def record_energy(
        self, timestamp: float, battery_soc: float, power_w: float, current_a: float
    ):
        """记录能量数据"""
        ...
        if self.data["energy"]:
            ...
            with open(filepath, "w", newline="", encoding="utf-8") as f:
                writer = csv.DictWriter(
                    f, fieldnames=["timestamp", "battery_soc", "power_w", "current_a"]
                )
                writer.writeheader()
...
if __name__ == "__main__":
    ...
    for i in range(100):
        ...
        recorder.record_state(
            timestamp, {"position": [i * 0.01, 0, 0], "velocity": [0.1, 0, 0]}
        )
