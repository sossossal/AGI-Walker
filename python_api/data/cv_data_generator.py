...
from python_api.comm.godot_vision_client import GodotVisionClient
...
class CVDatasetAnnotation:
    ...
    def add_bbox_annotation(
        self, image_id: int, category_id: int, bbox: List[float], area: float
    ):
        """添加边界框标注"""
    ...
    def add_keypoint_annotation(
        self, image_id: int, keypoints: List[float], num_keypoints: int
    ):
        """添加关键点标注"""
        self.annotations.append(
            {
...
class CVDataGenerator:
    ...
    def __init__(
        self,
        output_dir: str = "data/cv_dataset",
                ...
                for key, image in images.items():
                    ...
                    if "rgb" in key.lower():
                        filename = (
                            episode_dir
                            / "rgb"
                            / f"frame_{step_id:06d}_view_{view_id}.png"
                        )
                        cv2.imwrite(
                            str(filename), cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
                        )
                        ...
                        self.coco_annotation.add_image(
                            image_id, rel_filename, image.shape[1], image.shape[0]
                        )
...
if __name__ == "__main__":
    ...
    generator = CVDataGenerator(
        output_dir="data/test_cv_dataset", godot_host="localhost", godot_port=9999
    )
    ...
    if generator.connect_to_godot():
        ...
        generator.generate_episode_images(
            episode_id=0, trajectory=trajectory, save_interval=10  # 每10帧保存一次
        )
