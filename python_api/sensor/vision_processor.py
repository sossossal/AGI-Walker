...
class VisionProcessor:
    ...
    def _detect_edges(self, gray: np.ndarray) -> dict:
        """边缘检测"""
        edges = self.cv2.Canny(gray, self.config.canny_low, self.config.canny_high)

        # 统计边缘密度
        edge_density = np.sum(edges > 0) / edges.size

        return {"density": float(edge_density), "shape": edges.shape}

    def _detect_obstacles(self, edges: dict, frame: np.ndarray) -> List[dict]:
        """障碍物检测"""
        # 重新计算边缘（用于轮廓检测）
        gray = self.cv2.cvtColor(frame, self.cv2.COLOR_BGR2GRAY)
        edge_img = self.cv2.Canny(gray, self.config.canny_low, self.config.canny_high)
    ...
    def detect_obstacles_simple(self, frame: np.ndarray) -> List[dict]:
        ...
        # 查找轮廓
        contours, _ = self.cv2.findContours(
            cleaned, self.cv2.RETR_EXTERNAL, self.cv2.CHAIN_APPROX_SIMPLE
        )
