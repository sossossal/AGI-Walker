...
class SensorModel:
    ...
    def __init__(
        self, sensor_type: SensorType, noise_std: float = 0.01, delay_ms: float = 0
    ):
...
class SensorFusion:
    ...
    def process_imu(self, gyro: np.ndarray, accel: np.ndarray, dt: float):
        ...
        if imu_sensor:
            ...
            # 检查有效性
            if not np.any(np.isnan(gyro_reading.value)) and not np.any(
                np.isnan(accel_reading.value)
            ):
                # 互补滤波更新姿态
                self.complementary_filter.update(
                    gyro_reading.value, accel_reading.value, dt
                )
    ...
    def get_fusion_report(self) -> str:
        ...
        report.append("\n融合状态:")
        report.append(
            f"  位置: [{self.position[0]:.3f}, {self.position[1]:.3f}, {self.position[2]:.3f}] m"
        )
        report.append(
            f"  速度: [{self.velocity[0]:.3f}, {self.velocity[1]:.3f}, {self.velocity[2]:.3f}] m/s"
        )
        report.append(f"  速率: {np.linalg.norm(self.velocity):.3f} m/s")
        report.append(
            f"  姿态: Roll={np.degrees(self.orientation[0]):.1f}°, "
