...
class GodotVisionClient:
    ...
    def __init__(
        self, host: str = "localhost", port: int = 9999, timeout: float = 30.0
    ):
        """
        初始化客户端
    ...
    def send_command(self, command: Dict) -> Optional[Dict]:
        ...
        if not self.connected:
            raise ConnectionError(
                "Not connected to Godot server. Call connect() first."
            )
    ...
    def update_robot_state(
        self, position: List[float], orientation: List[float], joint_angles: List[float]
    ) -> bool:
        """
        更新机器人状态
    ...
    def capture_images(
        self, cameras: Optional[List[int]] = None
    ) -> Dict[str, np.ndarray]:
        """
        捕获所有视角的图像
        ...
        返回:
            图像字典 {名称: numpy数组}
        """
        command = {"command": "capture", "cameras": cameras}
    ...
    def set_camera_params(
        ...
        command = {"command": "set_camera", "camera_id": camera_id, "params": params}

        response = self.send_command(command)
        return response is not None and response.get("status") == "success"

    def set_environment(
        self, environment_type: str = "flat", lighting: str = "day"
    ) -> bool:
        """
        设置环境
