...
def demo_behavior_cloning():
    ...
    expert_demos = bc.collect_expert_demos(
        expert, n_episodes=50, save_path="expert_demos_bc.pkl"
    )

    # 步骤 3: 训练 BC
    print("\n[3/4] 训练行为克隆...")
    bc.train(expert_demos, n_epochs=10, batch_size=64)
...
def demo_gail():
    ...
    expert_demos = gail.collect_expert_demos(
        expert, n_episodes=100, save_path="expert_demos_gail.pkl"
    )

    # 步骤 3: GAIL 训练
    print("\n[3/4] GAIL 训练...")
    gail.train(expert_demos, total_timesteps=500_000, n_disc_updates_per_round=4)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--method", choices=["bc", "gail", "compare"], default="bc", help="演示方法"
    )
