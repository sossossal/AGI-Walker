...
def demo_parameter_sweep():
    ...
    for power in power_values:
        ...
        rating = (
            "优秀"
            if result["total_reward"] > 200
            else ("良好" if result["total_reward"] > 100 else "一般")
        )
        print(
            f"{power:<12.2f} {result['total_reward']:<12.2f} {result['steps']:<8} {rating:<15}"
        )
...
def demo_comparison():
    ...
    configurations = [
        {"name": "默认配置", "params": {}},
        {"name": "高功率配置", "params": {"motor_power_multiplier": 1.5}},
        {
            "name": "高刚度配置",
    ...
    for config in configurations:
        ...
        avg_steps = int(
            np.mean([controller.run_episode(max_steps=500)["steps"] for _ in range(3)])
        )
...
def main():
    ...
    parser.add_argument(
        "--demo", type=int, default=0, help="运行指定示例 (1-5), 0=全部"
    )
