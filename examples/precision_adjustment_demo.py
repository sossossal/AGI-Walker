...
def demo_motor_precision():
    ...
    print("-" * 70)
    print(
        f"{'设定值(W)':<12} {'实际值(W)':<12} {'扭矩(Nm)':<15} {'成本($)':<12} {'验证':<10}"
    )
    print("-" * 70)
    ...
    for value in test_values:
        ...
        if result["success"]:
            ...
            print(
                f"{value:<12.1f} {actual_value:<12.1f} {torque:<15.3f} ${cost:<11.2f} {valid:<10}"
            )
...
def demo_joint_precision():
    ...
    for value in test_values:
        ...
        if result["success"]:
            ...
            print(
                f"{value:<15.1f} {actual_value:<15.1f} {bandwidth:<20.3f} {valid:<10}"
            )
