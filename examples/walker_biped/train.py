...
def train_basic(config, timesteps=100000):
    ...
    model = PPO("MlpPolicy", env, verbose=1, **config["training_config"])
...
def train_full(config, timesteps=1000000, domain_randomization=False, n_envs=4):
    ...
    checkpoint_callback = CheckpointCallback(
        save_freq=10000, save_path="./checkpoints/", name_prefix="walker"
    )
    ...
    # 保存
    save_name = (
        "walker_policy_full_dr" if domain_randomization else "walker_policy_full"
    )
...
def main():
    ...
    parser.add_argument(
        "--mode", choices=["basic", "full"], default="basic", help="训练模式"
    )
    parser.add_argument("--timesteps", type=int, default=None, help="训练步数")
    parser.add_argument(
        "--domain-randomization", action="store_true", help="启用域随机化"
    )
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="walker_policy_full.zip",
        help="模型路径（测试时使用）",
    )
