...
def demo_different_environments():
    ...
    for env_name, env in environments:
        ...
        print(
            f"{env_name:<15} {success:<8} {distance:<12.2f} {max_tilt:<12.1f}° {评价:<20}"
        )
...
def demo_parameter_tuning_for_slopes():
    ...
    print("-" * 70)
    print(
        f"{'配置':<15} {'功率':<10} {'刚度':<10} {'成功':<8} {'距离(m)':<12} {'评价':<15}"
    )
    print("-" * 70)
    ...
    for config_name, params in configs:
        ...
        print(
            f"{config_name:<15} {params['motor_power_multiplier']:<10.1f} "
...
def demo_balance_control():
    ...
    for config_name, stiffness, damping in balance_configs:
        ...
        print(
            f"{config_name:<20} {stiffness:<10.1f} {damping:<10.1f} "
