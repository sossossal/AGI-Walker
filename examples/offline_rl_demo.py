...
def main():
    ...
    dataset = collector.collect_from_policy(
        expert_policy, n_episodes=N_EPISODES, deterministic=True
    )
    ...
    trainer = OfflineRLTrainer(
        env_id=ENV_ID, algorithm="cql", actor_lr=3e-4, critic_lr=3e-4
    )
    ...
    trainer.train_offline(
        mdp_dataset, n_steps=100000, save_interval=10000, save_dir="offline_models/cql"
    )
    ...
    # Fine-tuning
    trainer.finetune_online(env, n_steps=10000, save_dir="finetuned_models")
