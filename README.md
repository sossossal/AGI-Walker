...
AGI-Walker 是一个面向机器人建模、参数优化、自动化任务编排�?Godot 仿真的集成开发平台。项目已完成 V1 阶段的全面闭环，提供从参数化设计到物理仿真验证的完整链路�?
## 核心能力 (V1)

- **模块化建�?*: 快速创建双�?四足/轮式机器人模型，支持参数化定制�?- **参数自动优化**: 使用遗传算法或梯度下降优化重心分布、关节限位与 PID 参数�?- **Workflow 编排**: 标准化的机器人设计流水线，支�?Mock/Real 双模执行与状态持久化�?- **Web 控制�?*: 直观的任务监控界面，集成实时日志、产物管理与 Godot 一键同步�?- **仿真集成**: �?Godot 深度集成，提供高可靠�?TCP 控制链路�?Headless 自动化测试能力�?- **运维与监�?*: 完整�?Nightly 专项回归系统，实时追�?Smoke / Distributed / Godot 链路健康度�?
## 仓库结构

- `agi_walker/`: Python 主包，包�?Skills 引擎、Workflow 编排器与 CLI 实现�?- `web_panel/`: FastAPI Web 控制台，集成 SSE 实时通信�?Nightly 运维看板�?- `godot_project/`: Godot 仿真工程，提供物理模拟与机器人加载环境�?- `tests/`: 包含全量 Smoke Test、集成测试与 Nightly 回归脚本�?- `docs/`: 包含 V1 状态宣言、CLI 指南、Web 面板指南与部署手册�?
## 代码质量标准 (v1.0.0)
...
| 指标 | 现状 | 说明 |
|------|------|------|
| **Logger 框架** | 93% 覆盖 | 统一的日志系统，支持日志级别控制和生产部�?|
...
- 📖 [MIGRATION_GUIDE.md](docs/MIGRATION_GUIDE.md) - 升级指南和最佳实�?- 📊 [CODE_QUALITY.md](docs/CODE_QUALITY.md) - 详细的代码质量报�?- 📋 [CHANGELOG.md](CHANGELOG.md) - 完整的变更清�?- 📢 [RELEASE_NOTES.md](RELEASE_NOTES.md) - v2.0 发版说明
- 🧭 [docs/CURRENT_STATUS.md](docs/CURRENT_STATUS.md) - 当前状态与历史文档边界

### 向后兼容�?
�?**100% 向后兼容** - 现有代码无需修改即可继续工作。这些改进仅添加功能，不删除或破坏现�?API�?## 项目结构

- `agi_walker/`
  Python 主包，包�?Skills 加载器、CLI、工作流编排与相关实现�?- `examples/`
  示例脚本和标准任务目录，例如楼梯攀爬任务�?- `web_panel/`
  Web 控制面板、WebSocket 协议处理和静态页面资源�?- `godot_project/`
  Godot 仿真工程�?- `python_api/`、`python_controller/`
  Python 侧接口与控制逻辑�?- `parts_library/`
  零件库与相关文档资源�?- `training/`、`sysid/`
  训练和系统辨识相关代码�?- `tests/`
  单元测试、集成验证脚本和基准测试�?
## 仓库里现在有什�?
### 1. Skills 系统

仓库内目前包�?3 个核�?Skills 目录�?
...
说明�?
- 这些 Skills 的实现和文档存在于仓库中�?- 当前首页不再把具体导入示例写成“开箱即用”，因为部分入口仍需要进一步整理和验证�?
### 2. 任务示例
...
注意�?
- 该示例会在终端输出包�?emoji 的文本；在部�?Windows GBK 终端中，直接运行可能触发编码错误�?- 更适合作为代码阅读和本地调试入口，而不是首页承诺的“零配置演示”�?
### 3. Web 控制面板
...
当前判断�?
- 面板代码、协议处理和前端页面资源已存在�?- `/static/workflows.html` 现在已经提供一条更完整�?workflow 控制台链路：
  - 后台发起 workflow run
  - 轮询 + SSE 接收状态更�?  - 查看步级进度、实时日志、故障诊�?  - 下载产物�?workflow log
  ...
  - �?run 详情里保留最近一�?Godot 交付结果，并显示会话状态接口、失败阶段和重试入口
  - �?`scope/status/mode/date/text` 检�?runs
  - �?runs 列表进行分页
  - 首页现在提供 `Nightly 运维页` 入口：`/static/nightly.html`
- 该部分依�?FastAPI、静态资源路径和 Godot 联动环境�?- 在显式配置可�?Godot 环境后，Web workflow 控制台与 `session_bridge` 官方链路已具备稳定闭环，并已通过真实 headless smoke 验证�?- 当前仍不应承诺“跨平台、零配置、任意外�?Godot 环境下都稳定可用”�?- 容器化部署现在区分默�?`web-panel` 核心镜像和可�?`web-panel-distributed` 变体，避免把分布式监控依赖强行耦合进默认启动路径�?- Docker 下默�?`web-panel` 暴露�?`http://localhost:8080`，可�?`web-panel-distributed` 暴露�?`http://localhost:8081`�?
...
默认保留策略�?
- 最多保�?`200` 条归�?runs
- 最多保�?`30` �?- runs 列表默认每页 `20` 条，最大每�?`100` �?
可通过环境变量覆盖�?
...
- 如果设置�?`AGI_WALKER_WEB_ENV_FILE`，优先读取该文件
- 否则尝试读取 `deployment/web_panel.env`
- 再否则回退�?`deployment/web_panel.env.example`

`/api/system/status` 现在会额外返�?`distributed_monitor` 状态，用于说明当前 Web 面板运行环境是否具备 Zenoh 分布式监控能力�?
分布式监控当前还支持 actor TTL 清理，默认会�?`30` 秒后清走长期未更新的 actor，可通过以下环境变量覆盖�?
- `AGI_WALKER_DISTRIBUTED_ACTOR_TTL_SECONDS`

如果配置�?GitHub Actions nightly 状态来源，首页系统状态卡现在还会显示专项回归摘要�?
- `Nightly 回归`
- `最近专项运行`
- `Smoke / Distributed / Godot`

对应后端数据来自 `/api/system/status` �?`nightly_regressions` 字段。启用所需环境变量�?
- `AGI_WALKER_GITHUB_REPO`
- `AGI_WALKER_GITHUB_TOKEN`（可选，私有仓库或提�?GitHub API 额度时使用）
- `AGI_WALKER_GITHUB_WORKFLOW_FILE`，默�?`.github/workflows/ci.yml`
- `AGI_WALKER_NIGHTLY_STATUS_CACHE_SECONDS`，默�?`300`

### 工作流产物追�?
- `.output/workflow_artifacts/{workflow_name}/` 下的 JSON 文件会记录每�?step �?inputs/outputs/状态，workflow run �?real executor 模式会自动产生；
- 每个 artifact 满足固定 schema（workflow/step/executor/action/status/inputs/output/mode/started_at/ended_at），写入前会进行字段检验，避免遗留不完整数据；
- workflow 日志（`.output/workflow_log_*`）包�?`artifact_path` �?`artifacts` 数组，方便自动化消费或运维人员定位对�?step�?- 这些产物也可以作为回归或 Godot session_bridge 的可信数据源，直接引用在 `.output` 目录下�?
如果需要查看最近几次专项运行、每�?job �?artifact 名称以及本地复现命令，可直接打开�?- `/static/nightly.html`
...
这部分已经形�?V1 官方链路�?
- `session_bridge` 是默认推荐路�?- workflow 产物可通过 `/api/workflows/runs/{run_id}/godot-sync` 正式送往 Godot
- Windows 本地环境下的真实 headless smoke 已于 `2026-04-02` 实测通过
- `legacy controller` 继续保留，但只作为兼容路�?
...
`/api/system/status` 会返回当�?Godot Agent backend 摘要，主控制台首页也会直接显示：

- 当前 backend 模式
- 资源模式（`skills` �?`templates`�?- roles / templates 计数
- 当前默认 `project_path`
- 当前 `history_file`
...
仍然保留为兼容别名；�?modern `godot-agent` backend 下，它们会投影到 templates，并在返回里标出 `compatibility_alias=true`�?
## 安装
...
说明�?
- 仓库内提供了安装脚本�?Dockerfile�?- 不同模块的依赖并不完全相同，涉及 Godot、硬件、训练环境时通常需要额外配置�?
## 最�?Smoke 验收

如果你想先确认仓库最核心�?CLI / workflow / Web 面板导入链路是否正常，建议先运行�?
```bash
...
- `skills list`
- `skills validate`
- `robot_creation_pipeline` �?mock workflow
- `robot_creation_pipeline` �?real workflow
- `web_panel.server` 导入�?`WsMessage(type="ping")` 向后兼容检�?- Godot Agent fake backend 的最�?Web/API smoke
- 如果检测到外部 `godot-agent` 目录，还会追�?modern backend �?`templates / plan / doctor / skills alias` smoke
- 如果显式设置 `AGI_WALKER_ENABLE_GODOT_HEADLESS_SMOKE=1`，还会追加真�?Godot headless integration smoke
...
如果你想显式指定 external `godot-agent` 目录�?smoke 使用�?
```bash
...
如果你的工作环境暂时没有安装 Godot，可使用 `tests/stubs/` 目录下提供的 `godot_stub.bat` + `godot_stub.py` 来模�?headless 进程。把 `GODOT_EXECUTABLE` 指向这份 `.bat`，其内部会调�?`python godot_stub.py` 并在 `--tcp-port` 指定的端口上开启模拟服务，从而在不依�?Godot 二进制的情况下也能跑�?headless smoke。例如：
...
- 在设�?`AGI_WALKER_GODOT_HEADLESS_SCENE=run_rl_server.tscn` 后，同一�?headless lifecycle smoke 也已通过
...
这比直接复用仓库根目录下已有 `.output/` �?`exports/` 更适合作为回归验证入口�?
仓库�?CI 现在还提供两个专项回归入口，并会�?nightly 中自动运行：
...
如果你需要查�?nightly 专项回归�?artifact 名称、本地复现命令和故障排查顺序，请直接参考：
...
2. [CLI 使用指南](docs/guides/CLI_GUIDE.md)（`agi_walker workflows ...` 现在等同�?`agi_walker skills workflows ...`�?3. [GUI 使用指南](docs/guides/GUI_GUIDE.md)
...
- 说明仓库目标和模块分�?- 给出真实存在的文档入�?- 区分 V1 官方入口与实�?扩展方向

它不再在首页直接承诺以下内容�?
- 未验证的导入示例一定可直接运行
- 跨平台、零配置、任意外�?Godot 环境下都稳定可用
- 所有路线图条目已经进入可演示状�?- 缺少落地证据的性能结论

另外，`docs/archive_and_reports/` �?`docs/architecture/` 中部�?2026-01 的文档属于历史阶段快照�?这些文档可用于理解演进过程，但不应直接作为当前“完成度”或“生产就绪度”的权威说明�?当前状态请优先参�?[docs/CURRENT_STATUS.md](docs/CURRENT_STATUS.md)�?
## 相关文档
