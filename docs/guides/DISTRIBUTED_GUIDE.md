...
*   `web-panel` 是默认的核心 Web 面板镜像，保证 workflow 控制台和基础页面能力可构建、可启动。
*   `web-panel-distributed` 是可选 profile，额外安装 `eclipse-zenoh`，用于需要在容器内直接启用 Zenoh 分布式监控的场景。

如需启动带分布式监控依赖的 Web 面板变体：
...
*   Workflow 控制台: `http://localhost:8080/static/workflows.html`
*   可选 `web-panel-distributed` 变体: `http://localhost:8081/static/index.html`

如需执行最小 Docker 分布式 smoke，可直接在仓库根目录运行：

```bash
python tests/run_distributed_smoke.py --build
```

该脚本会启用 compose 的 `distributed` 和 `smoke` profiles，并依次拉起：
...
其中 `mock-godot` 是仅用于 smoke 验证的轻量 TCP 测试服务，用来替代真实 Godot 进程，确认 `sidecar-1 -> zenoh-router -> learner -> web-panel-distributed` 整条链路可以真正产生活跃 actor。验证通过后，可在：
...
*   `GET /api/system/status` 和 `GET /api/distributed/status` 现在会返回 `distributed_monitor` / `monitor` 状态字段，用于说明当前容器是否具备 Zenoh 监控能力。
*   `mock-godot` 只用于自动化 smoke，不应替代真实 Godot 部署路径。

### Configuration
*   **ZENOH_ROUTER**: Address of the Zenoh router (default `tcp/zenoh-router:7447`).
*   **GODOT_HOST**: Address of the Godot instance.
*   **AGI_WALKER_GODOT_HOST**: Compose/Smoke 中 Sidecar 使用的 Godot 主机名；分布式 smoke 默认会指向 `mock-godot`。
*   **AGI_WALKER_SIDECAR_ACTOR_ID**: 分布式 smoke 使用的 actor 标识，默认 `actor_docker_1`。
*   **Web Panel pagination/archive policy**: `deployment/web_panel.env.example` is loaded into the `web-panel` service by default.
*   **AGI_WALKER_ZENOH_ENDPOINT**: Web 面板分布式监控连接地址。默认 compose 配置会指向 `tcp/zenoh-router:7447`。
*   **AGI_WALKER_DISTRIBUTED_ACTOR_TTL_SECONDS**: Web 面板保留 actor 的最大空闲秒数，默认 `30`。
