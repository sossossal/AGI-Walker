...
and SHA-256 checksum. Artifact keys and documentation roles must be unique.
Bundle paths must be non-empty paths relative to the bundle root and may not
resolve outside that root, so the delivered directory remains self-contained.
Static-only bundles are valid when the readiness summary proves `static_only`;
stronger bundles can add `--live-smoke` and
