...
| 状态 | 含义 |
| --- | --- |
| `ready` | 真实字段齐备，必需 evidence 齐备，industrial step 不再等待外部输入 |
| `blocked` | 字段仍有占位符、必需 evidence 缺失，或 external-mainline 仍等待 industrial 输入 |

## 与客户现场 smoke 的关系

本报告只能说明“现场输入和证据归档面已闭合”。若目标是工业级交付签收，还必须继续执行客户现场真实设备 smoke，并把 `test_env/customer_site_live_smoke/customer_site_live_smoke_report.json` 作为独立 evidence 归档。执行说明见 `docs/guides/CUSTOMER_SITE_REAL_DEVICE_SMOKE_20260427.md`。
