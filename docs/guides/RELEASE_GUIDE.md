...
`tools/collect_release_evidence.py` 现在还会默认生成 `extension_execution_evidence`、`extension_execution_instance`、`extension_execution_schedule`、`extension_execution_actuals`、独立的 `customer_external_bindings_confirmation_report`，以及 `vulnerability_exception_review_report`。后者会把 review-due / expired exception 列表、follow-up 是否必需、以及最早到期时间固化成结构化 release evidence，供 `run_security_release_preflight.py` 和 `customer_acceptance_bundle` 直接消费。如果仓库中存在受管输入 `deployment/customer_delivery.external_bindings.json`，它会自动把这份配置接到 actuals 生成链，并在 actuals 之后追加 external bindings confirmation report。最新 collector 还支持 `--release-ops-execution-report-source`，用于把已有 `release_ops_execution_report.json` 复制进 canonical `test_env/release_evidence/operations/`；也支持 `--control-comm-closeout-source`，用于把已有 `control_comm_simulation_closeout.json` 复制进 canonical `test_env/release_evidence/control_communication/`，供 release bundle 的 `--control-comm-closeout` 使用。它只负责收集已有 wrapper，不会绕过 `release_ops` 的 `policy_profile` 边界主动执行高风险 action，也不会把非 live 控制/通信模拟当作真实硬件验证。

Phase 3 控制/通信 closeout 的推荐交付链是先生成或取得 closeout，再由 canonical collector 复制到 release evidence 树，最后把这条 canonical 路径传给 dynamic Godot release bundle。示例：

```bash
python tools/collect_release_evidence.py --output-root test_env/release_evidence --control-comm-closeout-source test_env/control_comm_simulation_ci/control_comm_simulation_closeout.json
python tools/build_dynamic_godot_release_evidence_bundle.py --static-closeout test_env/static_godot_node_tree_manifest_ci/static_godot_node_tree_evidence_closeout.json --delivery-gate test_env/static_godot_node_tree_manifest_ci/gate.json --readiness-summary test_env/dynamic_godot_release_readiness.json --control-comm-closeout test_env/release_evidence/control_communication/control_comm_simulation_closeout.json --output-root test_env/dynamic_godot_release_evidence_bundle
...
这条链路只把 `control_comm_simulation_closeout.v1` 作为 optional `control_comm_closeout` artifact 打包；默认 docs 集合会同时加入 optional `control_comm_workflow` 文档角色，指向 `docs/guides/CONTROL_COMMUNICATION_SIMULATION.md`。如果发布脚本显式传入 `--doc`，则文档集合由调用方完全负责。bundle validator 仍会要求 closeout 保持 `evidence_level=non_live_simulation` 且 live hardware release gate 为 `blocked`。
