...
- 在显式配置可用 Godot 环境后，`session_bridge` 官方链路已经通过真实 headless smoke 验证。
- 如果在 Windows 终端中遇到编码问题，优先使用 UTF-8 终端。
- Web 服务的系统状态接口现在会额外报告 `distributed_monitor` 能力状态，用于说明当前运行环境是否具备 Zenoh 分布式监控能力。
- 如果配置了 GitHub Actions nightly 状态来源，首页系统状态卡还会显示 nightly 专项回归摘要。

## 当前页面入口

- `http://localhost:8000/static/index.html`
  主控制台，包含任务、Godot 相关入口和 workflow 控制台跳转。
- `http://localhost:8000/static/workflows.html`
  workflow 控制台，面向 workflow 执行、状态查看和历史检索。
- `http://localhost:8000/static/nightly.html`
...
- workflow 控制台支持后台 workflow run、SSE 状态流、实时日志、故障诊断和历史 runs 检索
- workflow 控制台现在支持把机器人配置产物通过正式后端路由送往 Godot
- `session_bridge` 现在是 workflow -> Godot 官方推荐路径，legacy controller 主要保留兼容用途
- Godot Agent 集成层现在支持 backend 工厂切换，可在 legacy `godot_studio_agent` 与外部 `godot-agent` 后端之间切换
...
- 列出可用 workflows
- 以 `real/mock` 模式触发后台 workflow
- 显式选择执行策略：`force` / `resume`
- 使用 `output_root` 隔离产物目录
- 查看步级进度、实时日志、故障诊断和最终 JSON 结果
- 下载 artifacts、workflow log 和完整实时日志
- 将 `robot_config` 类 artifacts 送往 `legacy_controller` 或 `session_bridge`
- 使用 `/api/workflows/runs/{run_id}/godot-sync` 自动选择推荐机器人配置产物并送往 Godot
- 记录最近一次 Godot 交付结果，并在 run 详情里展示 transport/session/schema 状态
- run 详情现在还会显示 Godot session 实时状态、失败阶段、重试提示和重试入口
- 在 workflow 完成后可由前端自动触发 “送往 Godot”
- 通过 `scope/status/mode/date/text/only_failures` 检索当前 runs 与归档 runs
- 对 runs 列表分页浏览

说明：

- 当前 workflow run 仍由 Web 服务进程管理，适合调试、演示和最小操作闭环。
- 在显式配置 Godot 环境后，workflow -> `session_bridge` -> Godot 这条官方主线已经达到 V1 验收预期。
- 这仍不等于“跨平台、零配置、任意外部 Godot 环境”都已经达到统一稳定产品状态。
...
- `AGI_WALKER_WEB_ARCHIVE_MAX_RUNS`：归档最多保留多少条 run，默认 `200`
- `AGI_WALKER_WEB_ARCHIVE_MAX_AGE_DAYS`：归档最多保留多少天，默认 `30`
...
- 直接运行 `python -m web_panel.server` 时，workflow API 会自动尝试读取这些文件
- `quick_start.sh` / `quick_start.bat` 现在也会在启动前显式设置 `AGI_WALKER_WEB_ENV_FILE`
...
- 默认 `web-panel`
  使用核心依赖集，优先保证 workflow 控制台和基础页面稳定启动。
- 可选 `web-panel-distributed`
  通过 compose `distributed` profile 启动，额外安装 `eclipse-zenoh`，用于需要容器内 Zenoh 分布式监控的场景。
...
- `python tests/run_distributed_smoke.py --build` 会启用 `distributed` 和 `smoke` profiles，并附带拉起测试专用的 `mock-godot`。
- 该脚本用于验证 `sidecar-1 -> zenoh-router -> learner -> web-panel-distributed` 是否真正打通。
- 验证通过后，可在 `http://localhost:8081/api/distributed/status` 看到活跃 actor。
- 脚本入口位于 [`tests/run_distributed_smoke.py`](../../tests/run_distributed_smoke.py)。

相关接口：

- `GET /api/system/status`
  返回 `distributed_monitor` 字段，说明 `zenoh_available`、`monitor_active`、`endpoint` 和最近错误。
...
- `AGI_WALKER_GITHUB_TOKEN`，可选
- `AGI_WALKER_GITHUB_WORKFLOW_FILE`，默认 `.github/workflows/ci.yml`
- `AGI_WALKER_NIGHTLY_STATUS_CACHE_SECONDS`，默认 `300`

如果你需要查看最近几次专项 run，而不仅是首页摘要，可使用：

- `GET /api/nightly/regressions?limit=6`

这个接口会返回：

- 最近几次 `schedule / workflow_dispatch` 专项回归
- 每个 run 的 `smoke / distributed-smoke / godot-headless-smoke` job 状态
- 对应 artifact 名称
- 推荐本地复现命令

分布式监控还会对长期未更新的 actor 做 TTL 清理。默认 TTL 为 `30` 秒，可通过以下环境变量调整：
...
这条链路面向“按会话启动 Godot 进程、通过 TCP 读取遥测/发送动作”，更接近 RL 或调试桥，同时也是当前 workflow -> Godot 的推荐官方路径。
...
- `launch / status / schema / load_robot / step / stop` 这条 lifecycle 已纳入真实 headless smoke 验收范围。
- `2026-04-02` 在 Windows 本地环境下，默认场景 `demo_generated_biped.tscn` 与 runner 场景 `run_rl_server.tscn` 都已通过这条 lifecycle 验收。
...
- `POST /pipeline`: 执行 Godot Agent 命令流水线
- `GET /roles`: 查看当前 backend 的角色矩阵
- `GET /api/godot-agent/templates`: 列出当前 backend 暴露的模板资源
- `GET /api/godot-agent/templates/{template_id}`: 获取单个模板详情
...
- `GET /api/godot-agent/history`: 获取最近任务历史
- `GET /api/godot-agent/doctor`: 运行 Godot Agent 环境自检
- `POST /api/godot-agent/launch`: 请求启动 Godot 编辑器

说明：

- `godot-agent` backend 的正式资源语义现在是 `templates`
- legacy `godot_studio_agent` backend 仍保留 `godot_skills`
- 为了不打断旧前端和旧脚本，`/api/godot_skills/*` 继续保留，但返回里会带 `compatibility_alias` 和 `source_kind`
- `/api/system/status` 现在会额外返回 `godot_agent` 摘要，首页状态卡会直接显示当前 backend、资源模式以及 roles/templates 计数
- `godot-agent` modern backend 现在会显式绑定默认 `project_path` 和 `history_file`，避免在未配置项目路径时回退到 Web 进程当前工作目录
...
- 如果未显式配置 `AGI_WALKER_GODOT_PROJECT_PATH`，modern backend 会默认使用仓库内的 `godot_project/`
- 如果未显式配置 `AGI_WALKER_GODOT_AGENT_HISTORY_FILE`，任务历史会默认写到 `.output/godot_agent_backend/task_history.json`
...
- `GET /api/workflows/runs/{run_id}/status`: 获取轻量状态快照
- `POST /api/workflows/runs/{run_id}/cancel`: 发送取消请求
- `GET /api/workflows/runs/{run_id}/events`: 订阅 SSE 事件流
- `GET /api/workflows/runs/{run_id}/live-log`: 获取完整实时日志文本
- `GET /api/workflows/runs/{run_id}/log`: 下载 workflow JSON log
- `GET /api/workflows/runs/{run_id}/artifacts/{artifact_index}`: 下载指定产物
- `POST /api/workflows/runs/{run_id}/artifacts/{artifact_index}/godot-load`: 将 `robot_config` 产物送往 Godot，可选 `legacy_controller` 或 `session_bridge`
- `POST /api/workflows/runs/{run_id}/godot-sync`: 自动选择推荐 `robot_config` 产物，并按官方路径送往 Godot

### 真实 Godot Headless Smoke

仓库现在保留了一条显式 opt-in 的真实 Godot headless smoke：
...
- Godot 进程诊断与 stdout/stderr tail
- schema 摘要和 step 样本
...
- 设置 `AGI_WALKER_GODOT_HEADLESS_SCENE=run_rl_server.tscn` 后复跑同一条 lifecycle smoke，也已通过

CI 中另外保留了一个 `godot-headless-smoke` job，它现在支持：
