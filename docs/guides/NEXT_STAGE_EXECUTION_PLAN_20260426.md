...
如果需要把当前 blocked readiness 转成现场执行清单，运行 `tools/build_next_stage_external_evidence_checklist.py`。该工具输出 `next_stage_external_evidence_checklist.v1`，只把 `blocker_details` / `action_plan` 整理成外部证据项；它不替代总 readiness，也不会把缺真实输入的路线标成 ready。每个 `items[]` 会保留该路线的 `artifact_path`、`evidence_commands`、`input_templates` 和 `guide_paths`，分别用于定位目标 artifact、重建 route artifact、填写真实输入和查阅执行前检查。若输出 `handoff_validation_errors` 或顶层 `validation_errors` 非空，说明清单自身 artifact 路径、命令、模板/指南路径、item 字段形状或 artifact 结构已漂移，应先修复仓内契约再交给现场执行。
...
- checklist 的 `items[].artifact_path` 必须是仓内相对路径；不要把现场机器绝对路径或仓外路径写进清单。
...
- 客户现场真实设备 smoke 的 `checks[].evidence_path` 必须是相对 customer-site smoke 输入 JSON 所在目录的文件路径，禁止绝对路径和 `..`；strict evidence 文件检查会按该目录解析并确认文件存在。
- operator delivery checklist 的模板 evidence path 和 `--set-evidence` 覆盖值必须是仓库根相对路径，禁止绝对路径和 `..`；生成器只读取仓内 JSON evidence。
