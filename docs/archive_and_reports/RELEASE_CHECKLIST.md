...
在使�?`main` 分支发布新版本前，请务必确认以下事项�?

## 1. 代码质量门禁 (CI Gates)
- [ ] **Lint Check**: `ruff` �?`black` 检查全通过 (�?`continue-on-error`)�?
- [ ] **Unit Tests**: 所有单元测试在 Ubuntu �?Windows 上均为绿色�?
- [ ] **Coverage**: 核心�?`agi_walker/` 覆盖率达到基�?(当前建议 > 60%)�?

## 2. 功能回归验证
- [ ] **URDF Export**: 运行 `verify_pipeline.py`，确保生成的 URDF 结构完整（含 Thigh/Shin）�?
- [ ] **Web Panel**: 启动 `server.py`，确�?`design.html` 能成功生成机器人�?
