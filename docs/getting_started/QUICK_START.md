...
```
第一步: 测试零件库（Python）     ← 5分钟
    ↓
第二步: 测试环境系统（Godot）    ← 10分钟
    ↓
第三步: 测试Python API（可选）   ← 15分钟
```
...
## 第三步: 测试 Python API（可选）🐍

### 3.1 准备工作

**注意**: 此步骤需要 Godot 仿真器运行并监听 TCP 端口 9999。
...
**解决**: 
1. 确保脚本正确附加到节点
2. 检查 Godot 输出面板（而不是调试器）
3. 确认脚本的 `_ready()` 函数被调用
...
1. **阅读详细文档**
   - [零件库使用指南](file:///d:/新建文件夹/AGI-Walker/PARTS_LIBRARY_GUIDE.md)
   - [物理环境增强指南](file:///d:/新建文件夹/AGI-Walker/PHYSICS_ENVIRONMENT_GUIDE.md)
   - [参数转换指南](file:///d:/新建文件夹/AGI-Walker/PARAMETER_CONVERSION_GUIDE.md)
