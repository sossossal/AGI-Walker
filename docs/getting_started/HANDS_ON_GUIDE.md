...
- [ ] **Step 2**: Python Gym 环境测试�?分钟�?
- [ ] **Step 3**: Godot 测试场景创建�?5分钟�?
- [ ] **Step 4**: 环境参数实验�?0分钟�?
...
2. 点击 "导入"
3. 浏览�?`d:\新建文件夹\AGI-Walker\godot_project\project.godot`
4. 点击 "导入并编�?
...
2. 选择 `添加子节点`
3. 搜索 `Node`
4. 添加后重命名�?`EnvironmentController`
...
3. 重命名为 `Ground`
4. 选中 `Ground`，右�?�?`添加子节点` �?`CollisionShape3D`
5. 选中 `CollisionShape3D`
   - 在检查器�?`Shape` 属性，点击 `<�?` �?`新建 BoxShape3D`
   - 点击 `BoxShape3D`，设�?`Size` �?`(20, 1, 20)`
6. 再添�?`MeshInstance3D` �?`Ground`
   - `Mesh` �?`新建 BoxMesh`
   - 设置 `Size` �?`(20, 1, 20)`
...
3. 重命名为 `TestCube`
4. 位置设置�?`(0, 5, 0)`
5. 添加 `CollisionShape3D` �?`MeshInstance3D`
...
1. 选中根节�?`TestEnvironment`
2. 附加脚本：`res://scripts/test_environment.gd`

### 3.4 保存场景

1. `场景` �?`保存场景`
2. 保存�?`res://scenes/test_environment.tscn`
...
3. **学习进阶功能**
   - 阅读 [ADVANCED_USAGE.md](file:///d:/新建文件�?AGI-Walker/ADVANCED_USAGE.md)
   - 创建自己的机器人
   - 尝试域随机化训练
...
**Q: Godot 找不到脚�?*
- **A**: 确保脚本路径正确，检�?`res://scripts/environment/` 目录
