...
class SysIDOptimizer:
    ...
    def objective_function(self, params):
        ...
        # 2. Run Simulation
        sim_data = self.sim.run_trajectory(duration=self.real_data["metadata"]["duration"])
        ...
        real_pos = np.array([m["state"]["joints"]["hip_left"] for m in self.real_data["measurements"]])
        sim_pos = np.array([m["state"]["joints"]["hip_left"] for m in sim_data["measurements"]])
    ...
    def optimize(self, initial_guess=[0.5, 0.1]):
        ...
        print(f"   Optimal Params: Friction={result.x[0]:.4f}, Damping={result.x[1]:.4f}")
        print(f"   Final Error: {result.fun:.6f}")
        return result.x

from sysid.simulator_interface import MockSimulator
...
if __name__ == "__main__":
    ...
    import os
    if not os.path.exists("sysid/data"): os.makedirs("sysid/data")
    with open("sysid/data/mock_real.json", "w") as f:
        json.dump({"metadata": {"duration": 1.0}, "measurements": ground_truth_data["measurements"]}, f)
