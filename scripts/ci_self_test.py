...
def self_test():
    ...
    for wf_file in workflows_dir.glob("*.yml"):
        with open(wf_file, "r", encoding="utf-8") as f:
            ...
            env = data.get("env", {})
            if "FORCE_JAVASCRIPT_ACTIONS_TO_NODE24" in env or "ACTIONS_ALLOW_USE_UNSECURE_NODE_VERSION" in env:
            ...
            if "actions/checkout@" in content and "actions/checkout@v6.0.2" not in content:
                issues.append("actions/checkout must use stable native @v6.0.2")
            if "actions/setup-python@" in content and "actions/setup-python@v6.2.0" not in content:
