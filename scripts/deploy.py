...
class HealthChecker:
    ...
    def should_rollback(self) -> bool:
        ...
        if error_rate > self.config.rollback_threshold_error_rate:
            logger.warning(f"错误率过高: {error_rate} > {self.config.rollback_threshold_error_rate}")
            return True
        
        if latency > self.config.rollback_threshold_latency:
            logger.warning(f"延迟过高: {latency} > {self.config.rollback_threshold_latency}")
            return True
