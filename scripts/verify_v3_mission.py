...
def run_real_world_mission():
    ...
    # 强制清理旧状态确保从头运行
    run_id = f"mission_test_{int(time.time())}"
    result = orchestrator.execute_task_graph(mission_graph, parameters={"output_root": "test_env/mission_data"})
    ...
    fake_sensor_data = {"sensors": {"imu": {"orient": [20.0, 0.0, 0.0]}}, "torso_height": 0.8}
    
    safe_action = controller.safety_supervisor.check_and_filter(fake_sensor_data, dangerous_action)
    
    if safe_action["motors"]["hip_left"] < dangerous_action["motors"]["hip_left"]:
        logger.info(f"✅ Safety Shell Active: Modified dangerous command {dangerous_action['motors']['hip_left']} -> {safe_action['motors']['hip_left']:.2f}")
    else:
        logger.warning(f"⚠️ Safety Shell did not modify the action. Result: {safe_action['motors']['hip_left']}. Check thresholds.")
    ...
    if len(kb.experiences) > 0:
        logger.info(f"✅ Evolution Loop Closed: Found {len(kb.experiences)} new experiences in memory.")
        # 尝试检索
        best_exp = kb.retrieve_experience(fake_sensor_data, top_k=1)
        if best_exp:
            logger.info(f"✅ RAG Success: Retrieved similar experience '{best_exp[0].id}' for current state.")
    else:
        logger.warning("⚠️ No new experiences indexed. Ensure TrajectoryRecorder flushed the data.")
