...
def stress_test_io_pressure(frames=2000, frequency_hz=1000):
    ...
    if max_lat > 1.0:
        logger.warning("⚠️ High latency spike detected during IO. Consider async flushing.")
    else:
...
def stress_test_fixed_adaptive_safety():
    ...
    for _ in range(15):
        ...
        if monitor.is_overloaded():
            logger.info(f"🔥 Safety Triggered! CPU Usage: {stats['cpu_percent']}% at {time.time() - start:.2f}s")
            triggered = True
            break
        time.sleep(0.5)
    ...
    if not triggered:
        logger.warning("Safety not triggered. Current CPU might be too powerful for 4 burner threads.")
