...
class PartsViewer:
        ...
        for motor in self.parts["motors"]:
            rows.append(
                [
                    ...
                    motor["specs"].get(
                        "max_torque", motor["specs"].get("holding_torque", "N/A")
                    ),
            ...
            if (robot_type == "biped" and "biped" in kit["type"]) or (
                ...
                for item in kit["parts_list"]:
                    ...
                    if part:
                        ...
                        category_stats[category]["cost"] += (
                            part["price"] * item["quantity"]
                        )

                for category, stats in category_stats.items():
                    print(
                        f"{self.categories.get(category, category):<20} "
                        f"数量: {stats['count']:<4} "
                        f"成本: ${stats['cost']:.2f}"
                    )
    ...
    def show_quantitative_summary(self):
        ...
        if joint_torques:
            print(
                f"关节扭矩范围: {min(joint_torques):.0f}Nm - {max(joint_torques):.0f}Nm"
            )
